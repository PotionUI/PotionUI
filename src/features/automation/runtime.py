"""
`AutomationRuntime` - CRUD, graph validation, and trigger lifecycle for the
...
class AutomationRuntime:
    def __init__(
        ...
        self._watch_registry = FilesystemWatchRegistry()
    ...
    def _build_trigger(self, automation_id: str, node: Dict[str, Any]) -> Optional[TriggerSource]:
        ...
        if node_type == "trigger.filesystem":
            return FilesystemTrigger(
                automation_id, node_id, config, enqueue, self._watch_registry,
        ...
        if node_type == "trigger.gpu_threshold":
            gpu_monitor = self.engine.services.gpu_monitor
            if gpu_monitor is None:
                logger.error(f"[AUTOMATION_MANAGER] trigger.gpu_threshold node {node_id}: no GpuMonitor configured")
                return None
            return ResourceTrigger(
                automation_id, node_id, config, enqueue, gpu_monitor,
