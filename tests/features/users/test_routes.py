...
class TestUserController:
    ...
    @pytest.fixture
    def user_controller(self, mock_user_manager):
        """UserController instance with mocked manager"""
        return UserController(mock_user_manager)

    # Test GET all users
    @pytest.mark.asyncio
    async def test_get_all_users_admin_success(self, user_controller, mock_user_manager, admin_user, sample_user):
        """Test successful retrieval of all users by admin"""
        mock_users = [sample_user, admin_user]
        mock_user_manager.get_all.return_value = mock_users
        ...
        mock_user_manager.get_all.assert_called_once()
    ...
    @pytest.mark.asyncio
    async def test_get_all_users_exception(self, user_controller, mock_user_manager, admin_user):
        """Test exception handling in get_all_users"""
        mock_user_manager.get_all.side_effect = Exception("Database error")
    ...
    # Test GET user by ID
    @pytest.mark.asyncio
    async def test_get_user_admin_success(self, user_controller, mock_user_manager, admin_user, sample_user):
        """Test admin can get any user"""
        mock_user_manager.get_by_id.return_value = sample_user

        response = await user_controller.get_user("test-user-123", current_user=admin_user)

        assert response.success is True
        assert response.data['id'] == "test-user-123"
        mock_user_manager.get_by_id.assert_called_once_with("test-user-123")

    @pytest.mark.asyncio
    async def test_get_user_own_profile_success(self, user_controller, mock_user_manager, sample_user):
    ...
    @pytest.mark.asyncio
    async def test_get_user_not_found(self, user_controller, mock_user_manager, admin_user):
        """Test user not found"""
        mock_user_manager.get_by_id.return_value = None
    ...
    @pytest.mark.asyncio
    async def test_create_user_admin_success(self, user_controller, mock_user_manager, admin_user, sample_user):
    ...
    @pytest.mark.asyncio
    async def test_create_user_username_exists(self, user_controller, mock_user_manager, admin_user):
        """Test create user with existing username"""
        mock_user_manager.create.side_effect = ValueError("Username already exists")
    ...
    @pytest.mark.asyncio
    async def test_create_user_email_exists(self, user_controller, mock_user_manager, admin_user):
        """Test create user with existing email"""
        mock_user_manager.create.side_effect = ValueError("Email already exists")
    ...
    @pytest.mark.asyncio
    async def test_create_user_invalid_account_type(self, user_controller, mock_user_manager, admin_user):
        """Test create user with invalid account type"""
        mock_user_manager.create.side_effect = ValueError("Invalid account type. Must be USER or ADMIN")
    ...
    @pytest.mark.asyncio
    async def test_update_user_admin_success(self, user_controller, mock_user_manager, admin_user, sample_user):
        ...
        mock_user_manager.update.return_value = updated_user
    ...
    @pytest.mark.asyncio
    async def test_update_user_own_profile_success(self, user_controller, mock_user_manager, sample_user):
        """Test user can update own profile"""
        mock_user_manager.update.return_value = sample_user
    ...
    @pytest.mark.asyncio
    async def test_update_user_non_admin_password_forbidden(self, user_controller, mock_user_manager, sample_user):
        ...
        assert exc_info.value.status_code == status.HTTP_403_FORBIDDEN
        mock_user_manager.update.assert_not_called()

    @pytest.mark.asyncio
    async def test_update_user_admin_password_still_allowed(self, user_controller, mock_user_manager, admin_user, sample_user):
    ...
    @pytest.mark.asyncio
    async def test_update_user_not_found(self, user_controller, mock_user_manager, admin_user):
        """Test update non-existent user"""
        mock_user_manager.update.side_effect = ValueError("User not found")
    ...
    @pytest.mark.asyncio
    async def test_update_user_account_type_non_admin_forbidden(self, user_controller, mock_user_manager, sample_user):
        """Test non-admin cannot change account type"""
        mock_user_manager.update.side_effect = ValueError("Only admins can change account type")
    ...
    @pytest.mark.asyncio
    async def test_update_user_no_updates(self, user_controller, mock_user_manager, admin_user):
        """Test update with no valid fields"""
        mock_user_manager.update.side_effect = ValueError("No valid fields to update")
    ...
    # Test DELETE user
    @pytest.mark.asyncio
    async def test_delete_user_admin_success(self, user_controller, mock_user_manager, admin_user, sample_user):
        ...
        mock_user_manager.delete.assert_called_once_with(
    ...
    @pytest.mark.asyncio
    async def test_delete_user_cannot_delete_self(self, user_controller, mock_user_manager, admin_user):
        """Test admin cannot delete themselves"""
        mock_user_manager.get_by_id.return_value = admin_user
        mock_user_manager.delete.side_effect = ValueError("Cannot delete your own account")
    ...
    @pytest.mark.asyncio
    async def test_delete_user_not_found(self, user_controller, mock_user_manager, admin_user):
        """Test delete non-existent user"""
        mock_user_manager.get_by_id.return_value = None
        mock_user_manager.delete.side_effect = ValueError("User not found")
    ...
    @pytest.mark.asyncio
    async def test_delete_user_delete_failed(self, user_controller, mock_user_manager, admin_user, sample_user):
        """Test delete user database failure"""
        mock_user_manager.get_by_id.return_value = sample_user
        mock_user_manager.delete.side_effect = ValueError("Failed to delete user")
    ...
    @pytest.mark.asyncio
    async def test_upload_avatar_success(self, user_controller, mock_user_manager, sample_user):
        ...
        mock_user_manager.upload_avatar.return_value = updated_user
        ...
        mock_user_manager.upload_avatar.assert_called_once_with(
            user_id="test-user-123",
    ...
    @pytest.mark.asyncio
    async def test_upload_avatar_bad_content_type(self, user_controller, mock_user_manager, sample_user):
        """Test uploading a non-image file is rejected with 400"""
        mock_user_manager.upload_avatar.side_effect = ValueError("Only image files are allowed")
    ...
    @pytest.mark.asyncio
    async def test_upload_avatar_oversize(self, user_controller, mock_user_manager, sample_user):
        """Test uploading an oversized file is rejected with 413"""
        mock_user_manager.upload_avatar.side_effect = ValueError("Avatar exceeds the 5MB size limit")
    ...
    @pytest.mark.asyncio
    async def test_delete_avatar_clears_column(self, user_controller, mock_user_manager, sample_user):
        ...
        mock_user_manager.delete_avatar.return_value = cleared_user
        ...
        mock_user_manager.delete_avatar.assert_called_once_with("test-user-123")
    ...
    @pytest.mark.asyncio
    async def test_get_avatar_unknown_filename_404(self, user_controller, mock_user_manager):
        """Test serving an unknown avatar filename returns a real 404"""
        mock_user_manager.resolve_avatar_path.side_effect = ValueError("Avatar not found")
    ...
    @pytest.mark.asyncio
    async def test_get_avatar_traversal_attempt_404(self, user_controller, mock_user_manager):
