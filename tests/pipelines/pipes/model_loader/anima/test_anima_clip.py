...
def test_lazy_encoder_never_resolved_on_a_full_embed_cache_hit():
    ...
    AnimaClipTextEncoder(device="cpu", model_fingerprint="fp-hit", te_loader=warm_loader).encode_prompt(
        "a fox", "", do_classifier_free_guidance=False
    )
    assert warm_loader.calls == 1

    cold_loader = _CountingLoader(fake)
    cond = AnimaClipTextEncoder(device="cpu", model_fingerprint="fp-hit", te_loader=cold_loader).encode_prompt(
        "a fox", "", do_classifier_free_guidance=False
    )
...
def test_lazy_encoder_resolved_once_for_a_batch_of_misses():
    ...
    enc = AnimaClipTextEncoder(device="cpu", model_fingerprint="fp-batch", te_loader=loader)
...
def test_lazy_encoder_property_caches_after_first_resolve():
    fake = _FakeEncoder()
    loader = _CountingLoader(fake)
    enc = AnimaClipTextEncoder(device="cpu", te_loader=loader)
...
def test_eager_encoder_bypasses_te_loader_entirely():
    ...
    enc = AnimaClipTextEncoder(fake, device="cpu", te_loader=_must_not_run)
    assert enc.encoder is fake
