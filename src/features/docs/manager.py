...
class DocsManager:
    """Builds the role-filtered documentation tree and resolves doc content."""

    def __init__(self, plugin_registry, base_docs_path: str = "docs"):
        self.plugin_registry = plugin_registry
        self.base_docs_path = Path(base_docs_path)

    # ---------------------------------------------------------------- tree

    def build_tree(self, is_admin: bool) -> Dict[str, Any]:
    ...
    def get_content(self, doc_id: str, is_admin: bool) -> Dict[str, Any]:
        """
        ...
        _frontmatter, markdown = _parse_frontmatter(markdown)
    ...
    def _find_record(self, doc_id: str) -> Optional[_DocRecord]:
        for record in self._collect_records():
            if record.id == doc_id:
                return record
        return None

    def _collect_records(self) -> List[_DocRecord]:
        records: List[_DocRecord] = []
        records.extend(self._collect_repo_docs())
        records.extend(self._collect_plugin_docs())
        records.extend(self._collect_live_docs())
        return records

    def _collect_repo_docs(self) -> List[_DocRecord]:
        records: List[_DocRecord] = []

        user_dir = self.base_docs_path / "user"
        if user_dir.is_dir():
            for md_path in sorted(user_dir.glob("*.md")):
                records.append(self._record_from_markdown(md_path, id_prefix="user", audience="user"))

        if self.base_docs_path.is_dir():
            # Top-level only - `docs/user/*.md` is handled above, not re-included here.
            for md_path in sorted(self.base_docs_path.glob("*.md")):
                records.append(self._record_from_markdown(md_path, id_prefix="dev", audience="developer"))
        ...
        for subdir, default_category in (("techniques", "Techniques"), ("models", "Models")):
            section_dir = self.base_docs_path / subdir
            if not section_dir.is_dir():
                continue
            paths = sorted(set(section_dir.glob("*.md")) | set(section_dir.glob("*/*.md")))
            for md_path in paths:
                rel = md_path.relative_to(self.base_docs_path).with_suffix("").as_posix()
                records.append(self._record_from_markdown(
    ...
    def _collect_plugin_docs(self) -> List[_DocRecord]:
        records: List[_DocRecord] = []

        for manifest in self.plugin_registry.get_enabled_plugins():
            ...
            for index, entry in enumerate(doc_entries):
                record = self._record_from_plugin_doc(manifest.id, plugin_dir, entry, index, seen_ids)
    ...
    def _collect_live_docs(self) -> List[_DocRecord]:
        return [
            _DocRecord(
