"""Durable recipe-run orchestration.
...
into the ``execute_current_step`` seam via a registered ``RecipeExecutorRegistry``
...
class RecipeRunError(Exception):
    """Base for recipe-run domain errors."""


class RecipeRunNotFound(RecipeRunError):
    pass


class IllegalRecipeRunTransition(RecipeRunError):
    """A requested status move is not permitted from the current status."""

    def __init__(self, src: RecipeRunStatus, dst: RecipeRunStatus):
        self.src = src
        self.dst = dst
        super().__init__(f"Illegal recipe-run transition {src.value} -> {dst.value}")


class RecipeExecutorNotConfigured(RecipeRunError):
    """The step-executor registry has not been wired yet."""


class ActiveRecipeRunExists(RecipeRunError):
    """A run is already holding the single instance-wide active slot.

    Only raised for callers that asked not to join it (`reuse_active=False`) -
    the onboarding wizard still reuses the active run so its POST stays
...
_ACTION_TARGET: Dict[str, RecipeRunStatus] = {
    "pause": RecipeRunStatus.PAUSED,
    "resume": RecipeRunStatus.RUNNING,
    "cancel": RecipeRunStatus.CANCELLED,
}
...
_ACTION_SOURCES: Dict[str, frozenset] = {
    ...
    "retry_step": frozenset({RecipeRunStatus.FAILED}),
}
...
#: method (`RecipeRunner.grant_consent`) rather than the generic state-only
...
class RecipeRunner:
    """State-machine-enforcing orchestrator over ``RecipeRunRepository``."""

    def __init__(self, repository: Optional[RecipeRunRepository] = None):
        self.repo = repository or RecipeRunRepository()
    ...
    def create_run(
        ...
        *,
        ...
        With `reuse_active=False` an active run raises `ActiveRecipeRunExists`
        instead - the admin path, which must not silently adopt a run started
        elsewhere.
        """
    ...
    def get_run(self, run_id: str) -> Optional[RecipeRun]:
        return self.repo.get_run(run_id)

    def get_run_or_raise(self, run_id: str) -> RecipeRun:
        run = self.repo.get_run(run_id)
        if run is None:
            raise RecipeRunNotFound(run_id)
        return run

    def list_attempts(self, run_id: str) -> List[RecipeStepAttempt]:
        return self.repo.list_attempts(run_id)

    def list_runs(self, recipe_id: Optional[str] = None, limit: int = 50) -> List[RecipeRun]:
        """Runs newest first, optionally narrowed to one recipe."""
        return self.repo.list_runs(recipe_id=recipe_id, limit=limit)

    def get_latest_completed_run(self, recipe_id: str) -> Optional[RecipeRun]:
        """The most recent COMPLETED run for `recipe_id`, if any - see
        `RecipeRunRepository.get_latest_completed_run`."""
    ...
    def transition(
        ...
        *,
        ...
        attempt to mutate it raises ``IllegalRecipeRunTransition`` - completed runs
        are immutable.
        """
        run = self.get_run_or_raise(run_id)
        if not is_legal_transition(run.status, target):
            raise IllegalRecipeRunTransition(run.status, target)
    ...
    def record_step_attempt(
        ...
        *same* logical attempt (interim progress ticks, then its terminal
        ...
        run = self.get_run_or_raise(run_id)
        if run.is_terminal:
            raise IllegalRecipeRunTransition(run.status, run.status)
    ...
    def apply_action(self, run_id: str, action: str) -> RecipeRun:
        ...
        if action not in VALID_ACTIONS:
            raise RecipeRunError(f"Unknown recipe-run action: {action}")
        run = self.get_run_or_raise(run_id)
        ...
        if action == "retry_step":
            ...
            if run.current_step:
                self.repo.insert_attempt(
                    run_id, run.current_step, RecipeStepStatus.RUNNING
                )
            return self.repo.update_run(
                run_id, status=RecipeRunStatus.RUNNING, clear_error=True
            )
    ...
    def resume_position(self, run_id: str) -> Optional[RecipeRun]:
    ...
    @property
    def executor_registry(self) -> Any:
        """The wired step-executor registry, or `None` before bootstrap wires
        one (a minimal test container may never do so)."""
    ...
    def execute_current_step(self, run_id: str) -> RecipeRun:
        ...
        if self._executor_registry is None:
            raise RecipeExecutorNotConfigured(
                "No recipe step-executor registry is wired (Phase 3)."
            )
        run = self.get_run_or_raise(run_id)
        return self._executor_registry.execute(self, run)

    def drive(self, run_id: str, *, max_steps: int = 50) -> RecipeRun:
        ...
        `RecipeExecutorRegistry.execute()` only ever advances one step per
        call and explicitly leaves "loop, and decide when to" to the caller
        (see its docstring) - this is that caller, used by the recipe-run routes
        ...
        if self._executor_registry is None:
            ...
            return run
        steps_run = 0
        while run.status in (RecipeRunStatus.PENDING, RecipeRunStatus.RUNNING) and steps_run < max_steps:
            ...
            # other today).
            if run.status == RecipeRunStatus.RUNNING and run.current_step == before:
                break
        return run
    ...
    def drive_async(self, run_id: str, *, max_steps: int = 50) -> None:
        ...
        (the recipe-run routes) should re-read the run via
        ...
        def _run() -> None:
            try:
                ...
                # `RecipeExecutorRegistry.execute` and turned into a FAILED
                ...
                logger.exception("Background recipe-run drive crashed for run '%s'", run_id)
                self._fail_run_after_crash(run_id)
            finally:
                with self._drive_lock:
                    self._driving.discard(run_id)

        threading.Thread(target=_run, name=f"recipe-run-drive-{run_id}", daemon=True).start()
    ...
    def grant_consent(self, run_id: str, step_key: str, *, granted_by: Optional[str] = None) -> RecipeRun:
        ...
        normal step success (see ``RecipeExecutorRegistry.advance_past``).
        """
        run = self.get_run_or_raise(run_id)
        if run.status != RecipeRunStatus.AWAITING_CONSENT:
            raise IllegalRecipeRunTransition(run.status, RecipeRunStatus.RUNNING)
        ...
        run = self.transition(run_id, RecipeRunStatus.RUNNING, current_step=step_key)
