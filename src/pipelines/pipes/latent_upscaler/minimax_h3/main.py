...
VRAM: evicts every foreign GPU-resident component before this pipe's own
VAE-encode + upsampler-forward work, same eviction idiom
``dit_placement.py`` uses. Unlike ``latent_upscaler/ltx``'s own
``_free_room_for_upscale``, a resident DiT is offloaded only when it would
not otherwise fit beside the upsampler (this pipe's own fit rule, same
``effective_free_vram_gb``/``minimum_inference_memory_gb`` idiom
``generator/video_minimax_h3``'s ``_place_vae`` uses) -- a two-stage Enhance
run's `latent` path hands this pipe a ~0.6GB upsampler right after
``generator_stage1`` sampled with the DiT resident, and evicting it here
just to bring it straight back for ``generator_stage2`` is a ~20s round
trip this fit check now avoids. The TE host-RAM release goes through the
same shared ``release_idle_te`` (``generator/txt2vid_ltx/main.py``) every
native family's generator already calls -- by the time this pipe runs,
``prompt_encoder``/the generator have usually already evicted it, so this
call is typically a no-op fallback, same as LTX's own belt-and-suspenders
call.
...
def _resize_cover_center_crop_01(frames: torch.Tensor, height: int, width: int) -> torch.Tensor:
    ...
    return chw.permute(1, 0, 2, 3).unsqueeze(0)
...
def _free_room_for_upscale(bundle: Any, device: str, models: Any = None) -> None:
    """Evict every foreign GPU-resident component before this pipe's own
    VAE-encode/upsampler-forward GPU work, plus fully unload the idle TE's
    host RAM -- see the module docstring. A resident DiT is only evicted
    when the upsampler would not otherwise fit beside it (same fit rule as
    ``generator/video_minimax_h3``'s ``_place_vae``) -- unlike
    ``latent_upscaler/ltx``'s own ``_free_room_for_upscale``, which always
    evicts a resident DiT unconditionally.
    """
    ...
    if not str(device).startswith("cuda"):
        get_profiler().mark(
            "h3_upscale.free_room", device=str(device), dit_was_resident=False, dit_offloaded=False,
    ...
    if dit_was_resident:
        ...
        need_gb = (getattr(bundle.upsampler, "estimated_vram_gb", None) or 0.0) + minimum_inference_memory_gb()
    ...
    own_models = tuple(
        m for m in (
            ...
            dit if dit_was_resident and not dit_offloaded else None,
        ) if m is not None
    )
    ...
    logger.debug(
        "[H3_UPSCALE] eviction pass: dit.device=%s, dit_offloaded=%s, allocated %.2fGB -> %.2fGB, te_unloaded_gb=%.2f",
        dit_dev, dit_offloaded, alloc0, alloc1, te_unloaded_gb,
    )
    get_profiler().mark(
        "h3_upscale.free_room", device=str(device), dit_was_resident=dit_was_resident, dit_offloaded=dit_offloaded,
