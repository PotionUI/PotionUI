...
Geometry is constrained on both axes. The canvas must be a multiple of 32 with an aspect ratio between 1:4 and 4:1, and the generator rejects anything else instead of snapping it — the released canvas is 1344×768 (short edge 768, area capped at 768·1344), and smaller canvases are the family's strongest speed lever (960×544 runs roughly 2.3× faster per step). The frame count *is* snapped, upward to the next `17n + 5` the video VAE can decode, which maps to `5n + 2` latent frames; the audio stream's length follows from it at 40 latents per second per channel, doubled for stereo. Clips run up to 15 seconds — the model's release notes suggest staying at 5s or above, but that is a recommendation, not a limit this engine enforces; the real floor is whatever the `17n + 5` lattice accepts (as low as 5 frames).
...
- **Low-VRAM sequence chunking (`seq_chunk_rows`) reuses a quantized attention projection's prepared weight across its chunks, but not the MLP's.** Each block's `qkv_proj`/`out_proj` weight preparation (dequant, or the opt-in fp8 GEMM gate's on-demand staging) is computed once per projection call and shared by every row-chunk of that call; the interleaved SwiGLU `fc1`/`fc2` loop deliberately keeps its per-chunk preparation (retaining both projections' operands for the whole loop would raise the streamed working set chunking exists to bound), so a quantized MLP still pays one dequant/stage per chunk. The reuse is also unavailable whenever streaming layer prefetch is on (`$NATIVE_STREAM_PREFETCH`): the prefetcher owns every streamed leaf through its own forward pre/post hooks, and a hooked leaf is deliberately sent down the ordinary per-chunk dispatch so those hooks still fire — so under prefetch a streamed attention projection pays per-chunk preparation exactly like the MLP. See `MiniMaxH3Model.forward`'s `seq_chunk_rows` docstring for the full contract.
