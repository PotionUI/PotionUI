"""011 seeds the `toggle_last_generations` `keybinding_defaults` row for every
...
class TestMigration011AddToggleLastGenerationsKeybinding(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.migration = _load_migration("011_add_toggle_last_generations_keybinding", self.db)
    ...
    def test_seeds_the_default_row(self):
        ...
        with self.db.get_connection() as conn:
            row = conn.execute(
                ...
                "FROM keybinding_defaults WHERE id = 'toggle_last_generations'"
            ).fetchone()
        self.assertIsNotNone(row)
        self.assertEqual(row[0], 'h')
    ...
    def test_baseline_alone_has_no_such_row(self):
        with self.db.get_connection() as conn:
            row = conn.execute(
                "SELECT id FROM keybinding_defaults WHERE id = 'toggle_last_generations'"
            ).fetchone()
    ...
    def test_idempotent(self):
        ...
        with self.db.get_connection() as conn:
            count = conn.execute(
                "SELECT COUNT(*) FROM keybinding_defaults WHERE id = 'toggle_last_generations'"
            ).fetchone()[0]
