"""
Tests for the models operations module.
"""
...
@pytest.fixture
def collaborators(mock_model_repository, mock_tag_repository, mock_plugin_registry, mock_user_attribute_repository):
    """Build a ModelIndexCollaborators bundle with mocked dependencies."""
    bundle = build_model_index_collaborators(
    ...
    bundle.catalog.scanner = MagicMock()
    bundle.catalog.scanner.get_indexing_status.return_value = {}
    return bundle
...
class TestListModels:
    ...
    def test_list_models_queries_availability_once_for_the_page(
        ...
        with patch('src.features.models.catalog.model_availability_repo') as mock_avail:
            collaborators.catalog.scanner.get_indexing_status.return_value = {'total_models_db': 3}
            mock_avail.backend_ids_by_model.return_value = {}
            mock_avail.has_any.return_value = True
...
class TestIndexing:
    ...
    def test_start_indexing(self, collaborators, mock_plugin_registry):
    ...
    def test_delete_model_success(self, collaborators, mock_model_repository, mock_plugin_registry):
        ...
        result = operations.delete_model(collaborators, 'test-id')

        assert 'removed from index' in result['message'].lower()
        mock_model_repository.delete.assert_called_once_with('test-id')

    def test_delete_model_not_found(self, collaborators, mock_model_repository):
        """Test deleting a model that doesn't exist."""
        mock_model_repository.get_by_id.return_value = None

        with pytest.raises(ModelNotFoundException):
            operations.delete_model(collaborators, 'nonexistent-id')

    def test_delete_model_blocked_by_hook(self, collaborators, mock_model_repository, mock_plugin_registry):
        ...
        with pytest.raises(ModelIndexingException) as exc_info:
            operations.delete_model(collaborators, 'test-id')
...
class TestProviderFetch:
    ...
    def test_fetch_provider_info_success(self, collaborators, mock_plugin_registry):
        ...
        with patch('src.features.providers.registry.get_provider_registry') as mock_svc:
            ...
            result = operations.fetch_provider_info(collaborators, 'civitai')

        assert result['status'] == 'running'
        assert result['provider'] == 'civitai'

    def test_fetch_provider_info_provider_not_found(self, collaborators):
        ...
        with patch('src.features.providers.registry.get_provider_registry') as mock_svc:
            ...
            with pytest.raises(ProviderFetchException) as exc_info:
                operations.fetch_provider_info(collaborators, 'nonexistent')

        assert 'not found' in str(exc_info.value).lower()

    def test_fetch_provider_info_provider_not_initialized(self, collaborators):
        ...
        with patch('src.features.providers.registry.get_provider_registry') as mock_svc:
            ...
            with pytest.raises(ProviderFetchException) as exc_info:
                operations.fetch_provider_info(collaborators, 'civitai')
...
class TestTagOperations:
    ...
    def test_update_model_tags_success(self, collaborators, mock_model_repository, mock_tag_repository):
        ...
        result = operations.update_model_tags(collaborators, 'model-id', ['tag-1', 'tag-2'])
    ...
    def test_update_model_tags_model_not_found(self, collaborators, mock_model_repository):
        """Test updating tags for non-existent model."""
        mock_model_repository.get_by_id.return_value = None

        with pytest.raises(ModelNotFoundException):
            operations.update_model_tags(collaborators, 'nonexistent-id', ['tag-1'])

    def test_update_model_tags_invalid_tag(self, collaborators, mock_model_repository, mock_tag_repository):
        ...
        with pytest.raises(InvalidTagException):
            operations.update_model_tags(collaborators, 'model-id', ['invalid-tag-id'])

    def test_update_model_description_success(self, collaborators, mock_model_repository):
        ...
        result = operations.update_model_description(collaborators, 'model-id', 'New description')

        assert 'updated successfully' in result['message'].lower()

    def test_update_model_prompting_guidance_success(self, collaborators, mock_model_repository):
        ...
        result = operations.update_model_prompting_guidance(collaborators, 'model-id', 'Use short tags')

        assert 'updated successfully' in result['message'].lower()
        mock_model_repository.update_prompting_guidance.assert_called_once_with('model-id', 'Use short tags')

    def test_update_model_prompting_guidance_not_found(self, collaborators, mock_model_repository):
        """Test updating prompting guidance for a non-existent model."""
        mock_model_repository.get_by_id.return_value = None

        with pytest.raises(ModelNotFoundException):
            operations.update_model_prompting_guidance(collaborators, 'nonexistent-id', 'Use short tags')

    def test_update_model_preview_registers_files_row_and_stores_files_url(
        self, collaborators, mock_model_repository, tmp_path, monkeypatch
    ):
        ...
        collaborators.metadata.settings.get_file_storage_directory.return_value = str(tmp_path)
        ...
        result = operations.update_model_preview(collaborators, 
            'model-id',
    ...
    def test_update_model_preview_rejects_path_traversal(
        self, collaborators, mock_model_repository, tmp_path
    ):
        ...
        with pytest.raises(ModelIndexingException):
            operations.update_model_preview(collaborators, 
                'model-id',
    ...
    def test_update_model_preview_clear_deletes_previous_file(
        self, collaborators, mock_model_repository, monkeypatch
    ):
        ...
        result = operations.update_model_preview(collaborators, 'model-id', None, user_id='admin-1')
    ...
    def test_update_model_preview_not_found(self, collaborators, mock_model_repository):
...
class TestUserAssignments:
    ...
    def test_get_user_model_assignments(self, collaborators, mock_model_repository):
        ...
        result = operations.get_user_model_assignments(collaborators, 'user-1')

        assert result['user_id'] == 'user-1'
        assert len(result['assignments']) == 1

    def test_assign_model_to_user_success(self, collaborators, mock_model_repository, mock_plugin_registry):
        ...
        result = operations.assign_model_to_user(collaborators, 'model-1', 'user-1')

        assert 'assigned' in result['message'].lower()
        mock_model_repository.assign_model_to_user.assert_called_once_with('model-1', 'user-1')

    def test_assign_model_to_user_fails(self, collaborators, mock_model_repository, mock_plugin_registry):
        ...
        with pytest.raises(ModelAssignmentException):
            operations.assign_model_to_user(collaborators, 'model-1', 'user-1')

    def test_unassign_model_from_user_success(self, collaborators, mock_model_repository, mock_plugin_registry):
        """Test unassigning a model from a user."""
        mock_model_repository.unassign_model_from_user.return_value = True

        result = operations.unassign_model_from_user(collaborators, 'model-1', 'user-1')

        assert 'unassigned' in result['message'].lower()

    def test_unassign_model_from_user_not_found(self, collaborators, mock_model_repository, mock_plugin_registry):
        """Test unassigning a model that's not assigned."""
        mock_model_repository.unassign_model_from_user.return_value = False

        with pytest.raises(ModelAssignmentException):
            operations.unassign_model_from_user(collaborators, 'model-1', 'user-1')


class TestDownloadAndIndex:
    """Tests for download and index operations."""

    def test_start_download_and_index(self, collaborators, mock_plugin_registry):
        """Test starting download and index."""
        result = operations.start_download_and_index(collaborators, 
            name='test-model',
    ...
    def test_start_download_and_index_blocked(self, collaborators, mock_plugin_registry):
        ...
        with pytest.raises(ModelDownloadException) as exc_info:
            operations.start_download_and_index(collaborators, 
                name='test-model',
                link='https://example.com/model.safetensors',
                size='1GB',
                sha256='abc123'
...
class TestAssignmentFailureIsExplained:
    ...
    and the collaborators used to relay that as "Model already assigned to user or
    ...
    def test_blank_ids_are_rejected_before_touching_the_database(self, collaborators, mock_model_repository):
        with pytest.raises(ModelAssignmentException) as exc:
            operations.assign_model_to_user(collaborators, '', 'user-1')
    ...
    def test_already_assigned_raises_the_idempotency_signal(self, collaborators, mock_model_repository):
    ...
    def test_unknown_model_is_named(self, collaborators, mock_model_repository):
        self._fail_insert(mock_model_repository)
        mock_model_repository.find_user_model_assignment.return_value = None
        mock_model_repository.get_by_id.return_value = None

        with pytest.raises(ModelAssignmentException) as exc:
            operations.assign_model_to_user(collaborators, 'ghost-model', 'user-1')

        assert "No model with id 'ghost-model'" in str(exc.value)
        assert not isinstance(exc.value, ModelAlreadyAssignedException)

    def test_unknown_user_is_named(self, collaborators, mock_model_repository):
        self._fail_insert(mock_model_repository)
        mock_model_repository.find_user_model_assignment.return_value = None
        mock_model_repository.get_by_id.return_value = Mock()  # model exists

        with pytest.raises(ModelAssignmentException) as exc:
            operations.assign_model_to_user(collaborators, 'model-1', 'ghost-user')
