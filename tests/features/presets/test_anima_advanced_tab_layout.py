"""The Anima txt2img Advanced tab groups its fields into named sections, the
same idiom Krea-2/Wan/LTX-2.5 use on their own Advanced tabs: a "Sampling"
section (steps/sampler/cfg, in that order) and a "Step cache (FBCache))"
section, both rendered with the `section` field type rather than a bare `row`
or a `group`.
...
def advanced_tab(loader, serializer):
    matches = [p for p in loader.presets if str(p.path).replace("\\", "/").endswith("presets/marketplace/Anima")]
    assert len(matches) == 1, f"Anima matched {len(matches)} presets"
    preset = matches[0]
    ...
    assert "Advanced" in titles, f"Anima txt2img tabs are {titles}"
...
def test_advanced_tab_top_level_is_named_sections(advanced_tab):
    ...
    assert [c.get("title") for c in top_level] == ["Sampling", "Step cache (FBCache)"]
