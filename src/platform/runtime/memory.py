...
class MemoryManager:
    ...
    def __init__(self, gpu_manager: 'GpuManager', settings_manager: 'SettingsManager'):
        """
        Initialize memory manager.

        Args:
            gpu_manager: GPU manager for VRAM monitoring (injected)
            settings_manager: Settings manager for configuration (injected)
        """
    ...
    def get_memory_strategy(self, vram_budget: Optional[float] = None) -> Literal["full_vram", "balanced", "conservative"]:
        ...
        Args:
            vram_budget: VRAM budget in GB (if None, will get from gpu_manager)

        Returns:
            str: Memory strategy name
        """
        if vram_budget is None:
            vram_budget = self.gpu_manager.get_vram_budget()
    ...
    def calculate_optimal_tile_size(
        ...
        Args:
            ...
            vram_budget: Available VRAM in GB (if None, will get from gpu_manager)
        ...
        full_image_vram = self.gpu_manager.estimate_vram_usage(width, height, model_type)
        ...
        for test_size in test_sizes:
            estimated_vram = self.gpu_manager.estimate_vram_usage(test_size, test_size, model_type)
    ...
    def calculate_optimal_tile_count(
        ...
        logger.debug(
            ...
            f"{self.gpu_manager.estimate_vram_usage(tile_width, tile_height, model_type):.1f}GB)"
        )
    ...
    def get_batch_size_recommendation(
        ...
        per_image_vram = self.gpu_manager.estimate_vram_usage(width, height, model_type)
