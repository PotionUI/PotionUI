# Derived from: microsoft/TRELLIS.2 (MIT) — trellis2/modules/sparse/spatial/basic.py
# Derived from: microsoft/TRELLIS.2 (MIT) — trellis2/modules/sparse/spatial/spatial2channel.py
"""Sparse spatial ops: octree-factor down/upsample and channel<->spatial
rearrange. All already pure torch upstream (unique/scatter_reduce/
repeat_interleave) — ported faithfully, no CONV-backend branches to strip."""
...
__all__ = ["SparseDownsample", "SparseUpsample", "SparseSpatial2Channel", "SparseChannel2Spatial"]


class SparseDownsample(nn.Module):
    """Downsample a sparse tensor by `factor` via average (or max) pooling."""
    ...
    def forward(self, x: SparseTensor) -> SparseTensor:
        ...
        if cache is None:
            ...
            if self.training:
                ...
                out.register_spatial_cache("subdivision", subdivision)

        return out


class SparseUpsample(nn.Module):
    """Upsample a sparse tensor by `factor` via nearest-neighbor interpolation."""
...
class SparseSpatial2Channel(nn.Module):
    """Downsample a sparse tensor by `factor`, rearranging features from
    spatial to channel (octree-collapse: 8 children -> 1 parent x8 channels
    at factor=2)."""
    ...
    def forward(self, x: SparseTensor) -> SparseTensor:
        ...
        out_shape = None if x._shape is None else torch.Size([x._shape[0], x._shape[1] * self.factor**DIM])
        out = SparseTensor(new_feats.reshape(new_coords.shape[0], -1), new_coords, out_shape)
        out._scale = tuple(s * self.factor for s in x._scale)
        out._spatial_cache = x._spatial_cache
...
class SparseChannel2Spatial(nn.Module):
    """Upsample a sparse tensor by `factor`, rearranging features from
    channel back to spatial (octree-growth: the VAE decoders' mechanism for
    materializing children voxels from a parent's packed channels)."""
    ...
    def forward(self, x: SparseTensor, subdivision: Optional[SparseTensor] = None) -> SparseTensor:
        ...
        out_shape = None if x._shape is None else torch.Size([x._shape[0], x._shape[1] // self.factor**DIM])
        out = SparseTensor(new_feats, new_coords, out_shape)
        out._scale = tuple(s / self.factor for s in x._scale)
        if cache is not None:
            out._spatial_cache = x._spatial_cache
        return out
