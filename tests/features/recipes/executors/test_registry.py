"""`RecipeExecutorRegistry` - drives one run forward by exactly one step per
`execute()` call, against a real `RecipeRunner`/DB (mirrors
`test_runner.py`'s `file_db` pattern) and a small hand-built
...
def file_db(tmp_path):
    db_path = tmp_path / "setup_executor_registry.db"
...
class ReportsProgressExecutor:
    ...
    wires it in `RecipeExecutorRegistry.execute`."""
...
def test_advances_through_multiple_steps_to_completion(file_db):
    ...
    registry = RecipeExecutorRegistry(FakeCatalog(recipe), {"kind.one": step_one, "kind.two": step_two})

    manager = RecipeRunner()
    ...
    assert by_step["step-one"].status == RecipeStepStatus.SUCCEEDED
    assert by_step["step-two"].status == RecipeStepStatus.SUCCEEDED
...
def test_failing_step_moves_run_to_failed_with_error_detail(file_db):
    ...
    assert attempt.status == RecipeStepStatus.FAILED
...
def test_progress_reports_land_on_a_single_attempt_row_then_terminate(file_db):
    ...
    registry = RecipeExecutorRegistry(FakeCatalog(recipe), {"kind.one": step_one, "kind.two": AlwaysSucceedsExecutor()})
    manager = RecipeRunner()
    manager.register_executor_registry(registry)
    run = manager.create_run("fake-recipe")
    ...
    attempt = attempts[0]
    assert attempt.status == RecipeStepStatus.SUCCEEDED
...
def test_executor_that_never_reports_progress_is_unaffected(file_db):
    ...
    recipe = _two_step_recipe()
    registry = RecipeExecutorRegistry(FakeCatalog(recipe), {"kind.one": AlwaysSucceedsExecutor(), "kind.two": AlwaysSucceedsExecutor()})
    manager = RecipeRunner()
    manager.register_executor_registry(registry)
    run = manager.create_run("fake-recipe")
...
def test_unregistered_step_kind_fails_with_not_implemented(file_db):
    recipe = _two_step_recipe()
    registry = RecipeExecutorRegistry(FakeCatalog(recipe), {"kind.one": AlwaysSucceedsExecutor()})
    manager = RecipeRunner()
    manager.register_executor_registry(registry)
    run = manager.create_run("fake-recipe")
...
def test_executor_exception_is_contained_not_propagated(file_db):
    recipe = _two_step_recipe()
    registry = RecipeExecutorRegistry(FakeCatalog(recipe), {"kind.one": RaisesExecutor()})
    manager = RecipeRunner()
    manager.register_executor_registry(registry)
    run = manager.create_run("fake-recipe")
...
def test_recipe_not_found_fails_the_run(file_db):
    registry = RecipeExecutorRegistry(FakeCatalog(None), {})
    manager = RecipeRunner()
    manager.register_executor_registry(registry)
...
def test_awaiting_consent_parks_the_run_without_advancing(file_db):
    ...
    assert run.current_step == "step-one"
    attempt = manager.list_attempts(run.id)[0]
    assert attempt.status == RecipeStepStatus.AWAITING_CONSENT
    ...
    # only grant_consent (RecipeRunner) is allowed to move it on.
    with pytest.raises(RecipeRunError):
        manager.execute_current_step(run.id)
...
def test_grant_consent_completes_the_step_and_advances(file_db):
    ...
    attempts = [a for a in manager.list_attempts(run.id) if a.step_key == "step-one"]
    assert attempts[-1].status == RecipeStepStatus.SUCCEEDED
...
def test_recipe_with_no_steps_completes_immediately(file_db):
    ...
    registry = RecipeExecutorRegistry(FakeCatalog(recipe), {})
    manager = RecipeRunner()
    manager.register_executor_registry(registry)
