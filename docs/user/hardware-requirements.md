...
- **The model RAM cache keeps a floor of `min(max(8 GB, 10% of total RAM), 25% of total RAM)` free at all times** (`host_ram_reserve_gb()`, `src/platform/runtime/model_lifecycle/lifecycle.py`, enforced in `_make_room_for_ram`). That's 2 GB on an 8 GB box, 4 GB at 16 GB, 8 GB at 32 GB and above — it scales down on small machines instead of reserving a fixed 8 GB regardless of total RAM, but it's still a retention/eviction policy, not a guarantee that any particular model fits: don't count on cross-generation caching surviving a genuinely tight box.
