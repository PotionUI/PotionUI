...
def make_history_manager(current_tags=None, history=None):
    manager = MagicMock()
...
class TestListRecentFilters:
    ...
    must reach GenerationHistoryManager.get_history, not just `limit`."""

    @pytest.mark.asyncio
    async def test_filters_are_threaded_to_get_history(self):
        history_manager = make_history_manager(history={"generations": [], "total": 0})
        await OrganizeGalleryTool().execute(
            make_context(history_manager),
        ...
        _, kwargs = history_manager.get_history.call_args
    ...
    async def test_omitted_filters_pass_none_not_missing_generations(self):
        ...
        history_manager = make_history_manager(history={"generations": [], "total": 0})
        await OrganizeGalleryTool().execute(make_context(history_manager), operation="list_recent")
        _, kwargs = history_manager.get_history.call_args
...
class TestExecuteValidation:
    @pytest.mark.asyncio
    async def test_no_history_manager_errors(self):
    ...
    async def test_tag_without_tags_errors(self):
        ...
        result = await OrganizeGalleryTool().execute(
            make_context(history_manager), operation="tag", generation_id="gen-1",
        )
        assert result.success is False
    ...
    async def test_rate_out_of_range_errors(self):
        history_manager = make_history_manager()
        result = await OrganizeGalleryTool().execute(
            make_context(history_manager), operation="rate", generation_id="gen-1", rating=9,
        )
        assert result.success is False
...
class TestExecutePreviewDoesNotMutate:
    ...
    async def test_tag_preview_does_not_update_tags(self):
        history_manager = make_history_manager()
        result = await OrganizeGalleryTool().execute(
            make_context(history_manager), operation="tag", generation_id="gen-1", tags=["favorite"],
        )
        assert result.success is True
        history_manager.update_tags.assert_not_called()

    @pytest.mark.asyncio
    async def test_rate_preview_does_not_set_rating(self):
        history_manager = make_history_manager()
        result = await OrganizeGalleryTool().execute(
            make_context(history_manager), operation="rate", generation_id="gen-1", rating=4,
        )
        assert result.success is True
        history_manager.set_rating.assert_not_called()

    @pytest.mark.asyncio
    async def test_list_recent_returns_real_data(self):
        history_manager = make_history_manager(history={
            "generations": [{
        ...
        result = await OrganizeGalleryTool().execute(make_context(history_manager), operation="list_recent")
        assert result.success is True
...
class TestExecuteConfirmedUnexpectedError:
    ...
    async def test_unexpected_exception_names_tool_and_operation_not_bare_message(self):
        history_manager = make_history_manager()
        history_manager.set_rating.side_effect = RuntimeError("db exploded")
        result = await OrganizeGalleryTool().execute_confirmed(
            make_context(history_manager), operation="rate", generation_id="gen-1", rating=5,
        )
        assert result.success is False
...
class TestGetOperation:
    ...
    async def test_get_returns_full_untruncated_error(self):
        history_manager = make_history_manager()
        history_manager.get_by_id.return_value = {
        ...
        result = await OrganizeGalleryTool().execute(
            make_context(history_manager), operation="get", generation_id="gen-1",
        )
        assert result.success is True
        payload = json.loads(result.data)
        assert payload["error"] == "y" * 600
        history_manager.get_by_id.assert_called_once_with("gen-1", "user-1")
    ...
    async def test_get_without_generation_id_errors(self):
        history_manager = make_history_manager()
        result = await OrganizeGalleryTool().execute(make_context(history_manager), operation="get")
        assert result.success is False
...
class TestExecuteConfirmedRate:
    ...
    async def test_rate_calls_manager(self):
        history_manager = make_history_manager()
        history_manager.set_rating.return_value = 4
        result = await OrganizeGalleryTool().execute_confirmed(
            make_context(history_manager), operation="rate", generation_id="gen-1", rating=4,
        )
        assert result.success is True
        history_manager.set_rating.assert_called_once_with("gen-1", 4, "user-1")
    ...
    async def test_list_recent_via_confirmed_works_for_mcp_direct_path(self):
        ...
        history_manager = make_history_manager(history={"generations": [], "total": 0})
        result = await OrganizeGalleryTool().execute_confirmed(make_context(history_manager), operation="list_recent")
        assert result.success is True
