...
3. **Follow guided setup.** After the owner account is created you land on the **Setup** page, which answers "Is this instance ready to generate?". Before it offers any recipe, it asks where model files should live: point it at an external directory now, or **Skip** to keep the default `models/` location — either way you can change it later in **Administration → System Settings**. If a backend or model still needs attention, guided setup then walks you through it recipe by recipe — each step shows its status, and any step that needs to download something first asks you to **approve and download** (it lists exactly what it wants to fetch and the total size before it starts). When everything finishes, the page shows **"You're all set"** along with the test image your setup produced, and a **Create your first image** button.
