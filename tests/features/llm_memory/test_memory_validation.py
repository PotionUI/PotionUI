"""Content validation truth table for src.features.llm_memory.operations.write_note.
...
class TestSeedRejection:
    def test_global_rejects_seed_with_teaching_message(self):
        ...
        with pytest.raises(ValueError, match="one generation"):
            operations.write_note(repo, user_id="u1", key="k", content="generated a castle at seed 1234")
...
class TestParamDumpScopeAware:
    def test_global_rejects_bare_param_dump(self):
        ...
        with pytest.raises(ValueError, match="one generation"):
            operations.write_note(repo, user_id="u1", key="k", content="cfg 7, steps 30")
...
class TestProseAlwaysPasses:
    ...
    def test_prose_mentioning_one_param_keyword_passes(self):
        ...
        result = operations.write_note(repo, user_id="u1", key="k", content=content)
