...
<!-- Maintainer ruling (09-04): the vertical connector renders BEHIND the join
	...
	the line show through), with a downward arrowhead where the line meets the
	next card below: card -> block -> arrow -> card. -->
<div class="relative my-5 flex justify-center">
	<div class="absolute -bottom-5 -top-5 left-1/2 z-0 w-px -translate-x-1/2 bg-line-strong"></div>
	<div class="absolute left-1/2 z-0 -translate-x-1/2 text-fg-subtle" style="bottom: -26px">
		<ConsoleIcon name="chevron-down" class="h-3 w-3" />
	</div>
	...
	{:else}
		<div
			class="relative z-10 flex min-h-[34px] w-3/5 min-w-[300px] max-w-[680px] items-center gap-3 rounded-md border border-line bg-surface-2 px-3 py-1.5"
		>
