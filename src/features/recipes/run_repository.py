"""Persistence for recipe runs and their step attempts.
...
def _active_marker(status: RecipeRunStatus) -> Optional[int]:
...
class RecipeRunRepository:
    """Reads/writes ``setup_runs`` and ``setup_step_attempts``."""
    ...
    def insert_run(
        ...
        stored_input = dict(safe_input or {})
        stored_input[MODE_KEY] = mode
    ...
    def get_run(self, run_id: str) -> Optional[RecipeRun]:
        ...
        with db.get_cursor() as cursor:
            ...
            row = cursor.fetchone()
            return RecipeRun.from_row(row) if row else None

    def get_active_run(self) -> Optional[RecipeRun]:
    ...
    def get_latest_completed_run(self, recipe_id: str) -> Optional[RecipeRun]:
        ...
        with db.get_cursor() as cursor:
            cursor.execute(
                ...
                (recipe_id, RecipeRunStatus.COMPLETED.value),
            )
            row = cursor.fetchone()
            return RecipeRun.from_row(row) if row else None

    def list_runs(
        self, recipe_id: Optional[str] = None, limit: int = 50
    ) -> List[RecipeRun]:
        """Runs newest first, optionally narrowed to one recipe. Onboarding
        runs are included - they are recipe runs like any other."""
        sql = "SELECT * FROM setup_runs"
        params: List[Any] = []
        if recipe_id:
            sql += " WHERE recipe_id = ?"
        ...
        with db.get_cursor() as cursor:
            ...
            return [RecipeRun.from_row(r) for r in cursor.fetchall()]

    def update_run(
        self,
        run_id: str,
        *,
        status: Optional[RecipeRunStatus] = None,
        ...
        if status is not None:
            ...
            params.append(
                now_iso() if status in {RecipeRunStatus.COMPLETED} else None
            )
    ...
    def insert_attempt(
        ...
        *,
        ...
        with db.get_cursor() as cursor:
            ...
            return RecipeStepAttempt.from_row(cursor.fetchone())

    def update_attempt(
        self,
        attempt_id: str,
        *,
        status: Optional[RecipeStepStatus] = None,
        progress_current: Optional[int] = None,
        progress_total: Optional[int] = None,
        progress_unit: Optional[str] = None,
        safe_output: Optional[Dict[str, Any]] = None,
        ...
        that follows them (see ``RecipeRunner.record_step_attempt``'s
        ...
        if not sets:
            with db.get_cursor() as cursor:
                ...
                row = cursor.fetchone()
                return RecipeStepAttempt.from_row(row) if row else None
    ...
    def list_attempts(self, run_id: str) -> List[RecipeStepAttempt]:
        ...
        with db.get_cursor() as cursor:
            ...
            return [RecipeStepAttempt.from_row(r) for r in cursor.fetchall()]
