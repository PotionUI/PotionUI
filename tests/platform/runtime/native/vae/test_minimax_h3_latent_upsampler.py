...
``test_minimax_music3_dav_real_header_dry_run.py`` for the same pattern in
this repo). The header isn't part of the repo checkout (``ai/`` is
gitignored), so these tests skip rather than fail when it's absent --
construction/detection/forward coverage below stays tiny-config-only and
...
class TestAdaGNScaleShiftOrder:
    def test_scale_and_shift_are_not_swapped(self):
        """``emb_out.chunk(2, dim=1)`` must land as ``(scale, shift)``, not
        the reverse -- both orderings produce the SAME shapes and both pass
        every other test here, so this compares the real forward's output
        against an independently swapped computation rather than
        re-deriving the same formula forward() itself uses."""
        module = MiniMaxH3LatentUpsampler.from_config(_TINY_CONFIG, disable_weight_init)
        _randomize(module)
