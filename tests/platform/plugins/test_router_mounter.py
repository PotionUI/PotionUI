"""Tests for PluginRouterMounter (A6 dynamic router mount/unmount)."""
...
class TestPluginRouterMounter(unittest.TestCase):
    def setUp(self):
        ...
        self.app = FastAPI()
        self.router_mounter = PluginRouterMounter(loader=self.loader)
    ...
    def test_mount_registers_routes_and_they_respond(self):
        ...
        ok = self.router_mounter.mount(manifest, loader=self.loader)
        self.assertTrue(ok)
    ...
    def test_unmount_removes_only_that_plugins_routes(self):
        ...
        self.router_mounter.mount(manifest_a, loader=self.loader)
        self.router_mounter.mount(manifest_b, loader=self.loader)
        ...
        self.assertFalse(self.router_mounter.is_mounted("plugin-a"))
        self.assertTrue(self.router_mounter.is_mounted("plugin-b"))
    ...
    def test_double_mount_is_idempotent(self):
        ...
        self.assertTrue(self.router_mounter.mount(manifest, loader=self.loader))
    ...
    def test_unmount_never_mounted_plugin_is_noop(self):
        self.assertTrue(self.router_mounter.unmount("never-mounted"))

    def test_remount_after_unmount_works(self):
        self._create_plugin("plugin-a", API_MODULE_SOURCE.format(plugin_id="plugin-a"))
        manifest = self._manifest_for("plugin-a")

        self.router_mounter.mount(manifest, loader=self.loader)
        self.assertEqual(self.client.get("/api/plugins/plugin-a/ping").status_code, 200)
