...
class GenerationManager:
    ...
    def __init__(
            ...
            models=None,  # ModelLifecycleManager - central model/artifact cache
    ...
    def _inject_built_in_services(self, pipe_class: type, pipe_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        They are automatically injected by GenerationManager and don't need
        ...
        for input_spec in input_specs:
            ...
            if input_spec.io_type == IOType.SERVICE:
                ...
                if service_name == "GPU":
                    pipe_input[input_spec.name] = self.gpu_manager
                    ...
                    pipe_input[input_spec.name] = self.memory_manager
                    ...
                    pipe_input[input_spec.name] = self.settings_manager
    ...
    def generate(
        ...
        with self._run_lock:
            if self._running_generation_id is not None:
                raise RuntimeError(
                    f"GenerationManager is already running {self._running_generation_id}; "
        ...
        if generation_id:
            try:
                profile_dir = Path(self.settings_manager.get_file_storage_directory()) / "profiles" / generation_id
