...
- `device` — what the routed backend's hardware actually reports, read from that backend's own `resolve_execution_device()` (never guessed from its driver name, and never a bare class tag — a native backend can be configured for `cpu` or any `cuda:N`, so only the specific instance knows which): free/total VRAM from this host's own GPU monitor when the backend is configured for a GPU AND that GPU's index actually matches the one the monitor is bound to (`local`); `none` when there's definite evidence of no applicable GPU — a native backend explicitly configured with no GPU (`device="cpu"`), or this host genuinely has no GPU at all; `null` numbers for a remote worker whose hardware isn't this process's to read (`remote`); or `unknown` when the backend is configured for a GPU index this host's monitor doesn't match (a native backend on GPU 1 while this process only monitors GPU 0 — its reading is never borrowed for a different device), or hasn't declared where it executes at all (e.g. a `comfyui` backend, whose server has its own hardware this host doesn't see).
