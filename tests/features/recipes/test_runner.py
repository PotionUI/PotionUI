"""Durable recipe-run lifecycle against the real file-backed SQLite wrapper.
...
def test_create_after_terminal_makes_new_run(file_db):
    mgr = _manager()
    first = mgr.create_run("native-image-starter")
    mgr.transition(first.id, RecipeRunStatus.RUNNING)
    mgr.transition(first.id, RecipeRunStatus.COMPLETED)
    # The prior run is terminal, so a fresh create is allowed.
    second = mgr.create_run("native-image-starter")
    assert second.id != first.id
    assert second.status == RecipeRunStatus.PENDING
...
def test_legal_transition_chain(file_db):
    ...
    run = mgr.transition(run.id, RecipeRunStatus.RUNNING)
    assert run.status == RecipeRunStatus.RUNNING
    run = mgr.transition(run.id, RecipeRunStatus.AWAITING_CONSENT)
    assert run.status == RecipeRunStatus.AWAITING_CONSENT
    run = mgr.transition(run.id, RecipeRunStatus.RUNNING)
    run = mgr.transition(run.id, RecipeRunStatus.COMPLETED)
...
def test_illegal_transition_rejected(file_db):
    ...
    with pytest.raises(IllegalRecipeRunTransition):
        mgr.transition(run.id, RecipeRunStatus.COMPLETED)


def test_completed_run_is_immutable(file_db):
    mgr = _manager()
    run = mgr.create_run("r")
    mgr.transition(run.id, RecipeRunStatus.RUNNING)
    mgr.transition(run.id, RecipeRunStatus.COMPLETED)
    with pytest.raises(IllegalRecipeRunTransition):
        mgr.transition(run.id, RecipeRunStatus.RUNNING)
    with pytest.raises(IllegalRecipeRunTransition):
        mgr.transition(run.id, RecipeRunStatus.PAUSED)


def test_transition_on_missing_run_raises(file_db):
    with pytest.raises(RecipeRunNotFound):
        _manager().transition("nope", RecipeRunStatus.RUNNING)
...
def test_attempts_accumulate_per_step(file_db):
    ...
    a1 = mgr.record_step_attempt(run.id, "artifacts.fetch", RecipeStepStatus.FAILED)
    a2 = mgr.record_step_attempt(run.id, "artifacts.fetch", RecipeStepStatus.RUNNING)
    a3 = mgr.record_step_attempt(run.id, "models.index", RecipeStepStatus.RUNNING)
...
def test_record_step_attempt_with_attempt_id_updates_in_place(file_db):
    ...
    first = mgr.record_step_attempt(
        run.id, "artifacts.fetch", RecipeStepStatus.RUNNING,
    ...
    final = mgr.record_step_attempt(
        run.id, "artifacts.fetch", RecipeStepStatus.SUCCEEDED,
    ...
    assert final.status == RecipeStepStatus.SUCCEEDED
...
def test_attempt_rejected_on_terminal_run(file_db):
    ...
    run = mgr.create_run("r")
    mgr.transition(run.id, RecipeRunStatus.RUNNING)
    mgr.transition(run.id, RecipeRunStatus.CANCELLED)
    with pytest.raises(IllegalRecipeRunTransition):
        mgr.record_step_attempt(run.id, "any", RecipeStepStatus.RUNNING)
...
def test_pause_resume_cancel(file_db):
    ...
    assert run.status == RecipeRunStatus.PAUSED
    run = mgr.apply_action(run.id, "resume")
    assert run.status == RecipeRunStatus.RUNNING
    run = mgr.apply_action(run.id, "cancel")
    assert run.status == RecipeRunStatus.CANCELLED


def test_retry_step_reopens_failed_run_with_new_attempt(file_db):
    mgr = _manager()
    run = mgr.create_run("r")
    mgr.transition(run.id, RecipeRunStatus.RUNNING, current_step="artifacts.fetch")
    mgr.record_step_attempt(run.id, "artifacts.fetch", RecipeStepStatus.FAILED)
    mgr.transition(
        run.id, RecipeRunStatus.FAILED, error_code="DOWNLOAD_FAILED"
    )
...
def test_resume_after_fresh_manager_instance(file_db):
    ...
    first_mgr = RecipeRunner()
    run = first_mgr.create_run("r")
    first_mgr.transition(run.id, RecipeRunStatus.RUNNING, current_step="models.index")
    first_mgr.record_step_attempt(run.id, "models.index", RecipeStepStatus.RUNNING)

    # Simulate a restart: brand-new manager + repository, same database file.
    restarted = RecipeRunner()
    resumed = restarted.resume_position(run.id)
    assert resumed is not None
    assert resumed.status == RecipeRunStatus.RUNNING
...
def test_execute_current_step_raises_until_phase3(file_db):
    mgr = _manager()
    run = mgr.create_run("r")
    with pytest.raises(RecipeExecutorNotConfigured):
...
# `.execute(runner, run) -> RecipeRun` (see `execute_current_step`) -
# rather than a real `RecipeExecutorRegistry` + recipe/executors, since only
...
class _CrashingRegistry:
    ...
    exception (which `RecipeExecutorRegistry.execute` already catches and
...
def test_drive_async_runs_the_step_and_reaches_completed(file_db):
    ...
    completed = _wait_for_run(mgr, run.id, lambda r: r.status == RecipeRunStatus.COMPLETED)

    assert completed.status == RecipeRunStatus.COMPLETED
...
def test_drive_async_returns_before_the_step_finishes(file_db):
    ...
    assert in_flight.status in (RecipeRunStatus.PENDING, RecipeRunStatus.RUNNING)
...
def test_drive_async_without_executor_registry_is_a_noop(file_db):
    ...
    assert mgr.get_run(run.id).status == RecipeRunStatus.PENDING
...
def test_crashed_drive_marks_the_run_failed_not_stuck_running(file_db):
    ...
    mgr.drive_async(run.id)
    failed = _wait_for_run(mgr, run.id, lambda r: r.status == RecipeRunStatus.FAILED)

    assert failed.status == RecipeRunStatus.FAILED
...
def test_drive_async_recovers_the_driving_guard_after_a_crash(file_db):
    ...
    mgr.drive_async(run.id)
    _wait_for_run(mgr, run.id, lambda r: r.status == RecipeRunStatus.FAILED)
...
def test_mode_round_trips_without_a_column_of_its_own(file_db):
    """`mode` rides inside the run's `safe_input` JSON (no migration), and must
    come back out as the record's own field rather than leaking into the input
    a caller supplied."""
    mgr = _manager()

    run = mgr.create_run("r", mode=MODE_ADMIN, safe_input={"engine": "native"})

    assert run.mode == MODE_ADMIN
    assert run.safe_input == {"engine": "native"}
    assert mgr.get_run(run.id).mode == MODE_ADMIN


def test_mode_defaults_to_onboarding(file_db):
    mgr = _manager()

    run = mgr.create_run("r")

    assert run.mode == MODE_ONBOARDING


def test_create_without_reuse_conflicts_on_an_active_run(file_db):
    """The wizard's POST is idempotent (it joins the active run); the admin
    page must not silently adopt a run someone else started."""
    mgr = _manager()
    first = mgr.create_run("r")

    assert mgr.create_run("r").id == first.id
    with pytest.raises(ActiveRecipeRunExists):
        mgr.create_run("r", mode=MODE_ADMIN, reuse_active=False)


def test_list_runs_is_newest_first_and_filters_by_recipe(file_db):
    mgr = _manager()
    first = mgr.create_run("recipe-a")
    mgr.apply_action(first.id, "cancel")
    second = mgr.create_run("recipe-b")

    assert [r.id for r in mgr.list_runs()] == [second.id, first.id]
    assert [r.id for r in mgr.list_runs(recipe_id="recipe-a")] == [first.id]
