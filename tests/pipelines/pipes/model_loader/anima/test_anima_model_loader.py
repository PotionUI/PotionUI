...
def test_process_eagerly_acquires_only_vae_and_dit():
    """The TE's acquire() is deferred (see AnimaClipTextEncoder) -- process()
...
def test_te_acquire_deferred_until_encoder_is_read():
    models, out = _run(ModelLoaderAnimaPipe(config=_config()))
    clip = out.output["text_encoder"]
    _ = clip.encoder
    _ = clip.encoder  # a second read must not re-acquire
    te_calls = [k for k, _ in models.calls if k == "native/te//m/qwen3_06b.safetensors"]
    assert len(te_calls) == 1


def test_te_never_acquired_when_encoder_is_never_read():
    models, out = _run(ModelLoaderAnimaPipe(config=_config()))
    assert not any(k == "native/te//m/qwen3_06b.safetensors" for k, _ in models.calls)
    _ = out.output["text_encoder"].encoder
    assert any(k == "native/te//m/qwen3_06b.safetensors" for k, _ in models.calls)
...
def _fps(loras):
    models, out = _run(ModelLoaderAnimaPipe(config=_config(loras=loras)))
