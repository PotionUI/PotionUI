...
(`ChatManager`, `McpProtocolManager`, `PromptEnhancementManager`,
`ToolContext`) hold the repository and pass it in; nothing here is stored
across calls.

One concern (memory-note CRUD + content validation), small enough for a
single module - split it out before it outgrows ~200 lines rather than let a
second concern move in here.
"""
import logging
import re
...
def _validate_content(content: str, scope: str) -> None:
    ...
    if scope == "global" and _looks_like_param_dump(content):
        raise ValueError(f"Memory note rejected: {_ONE_GENERATION_MESSAGE}")
...
def write_note(
    ...
    _validate_content(content, scope)
...
def get_note(repository: LLMMemoryRepository, user_id: str, note_id: str) -> Optional[LLMMemoryNote]:
    ...
    """
    return repository.get_by_id(note_id, user_id)


def update_note(
    repository: LLMMemoryRepository, user_id: str, note_id: str, key: str, content: str
    ...
    """
    _check_length(content)
    return repository.update(note_id, user_id, key, content)
...
def delete_note(repository: LLMMemoryRepository, user_id: str, note_id: str) -> bool:
    """Delete a note by ID.
