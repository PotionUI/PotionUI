"""013 seeds the `toggle_workbench_panel` `keybinding_defaults` row for every
...
class TestMigration013AddToggleWorkbenchPanelKeybinding(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.migration = _load_migration("013_add_toggle_workbench_panel_keybinding", self.db)
    ...
    def test_seeds_the_default_row(self):
        ...
        with self.db.get_connection() as conn:
            row = conn.execute(
                ...
                "FROM keybinding_defaults WHERE id = 'toggle_workbench_panel'"
            ).fetchone()
    ...
    def test_baseline_alone_has_no_such_row(self):
        with self.db.get_connection() as conn:
            row = conn.execute(
                "SELECT id FROM keybinding_defaults WHERE id = 'toggle_workbench_panel'"
            ).fetchone()
    ...
    def test_idempotent(self):
        ...
        with self.db.get_connection() as conn:
            count = conn.execute(
                "SELECT COUNT(*) FROM keybinding_defaults WHERE id = 'toggle_workbench_panel'"
            ).fetchone()[0]
