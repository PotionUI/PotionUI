...
Three friendly presets are offered for the refine's denoise (Light 0.30 / Balanced 0.45 / Strong 0.60), plus an expert override on the Advanced tab for denoise, video sigma shift and step count directly. The lightx2v SLA turbo LoRA at ~0.5 strength is an optional accelerant for this pass, same as elsewhere in the family (see the sparse-attention discussion under "Sampling" below) — it is not required, and this repository's own 4-step turbo recipe is validated LoRA-free.
