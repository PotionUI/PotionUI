...
@pytest.fixture
def media_manager(settings_manager, local_root, bucket_driver):
    return MediaManager(
...
@pytest.fixture
def saved_audio(repos_on_test_db, settings_manager, bucket_driver, minimal_wav_file):
...
class TestGenerationMediaDriverBypassClosed:
    def test_get_generation_media_serves_from_the_driver(
        self, media_manager, saved_audio, local_root
    ):
    ...
    def test_get_file_by_id_serves_from_the_driver(
        self, media_manager, saved_audio
    ):
        ...
        result = media_manager.get_file_by_id(files[0].id)

        assert result.content == expected_bytes

    def test_delete_generation_media_deletes_through_the_driver(
        self, media_manager, saved_audio, bucket_driver
    ):
        ...
        result = media_manager.delete_generation_media(gen_id, user_id=user_id)
