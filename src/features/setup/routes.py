...
The run endpoints under `/api/setup/runs` are the onboarding wizard's view of
a recipe run (see `src.features.recipes`): same runner, same durable rows, with
`mode="onboarding"` so the whole recipe executes - including the steps a
recipe marks `onboarding_only`, which the admin Recipes page skips.
"""
...
from src.features.recipes.dto import (
    CreateRecipeRunRequest,
    RecipeRunActionRequest,
    RecipeRunView,
    RecipeSummary,
)
from src.features.recipes.records import MODE_ONBOARDING
from src.features.recipes.routes import run_view
...
def _require_admin_or_404(user: User) -> None:
    """Gate the wizard's run routes. Setup mutates the whole instance, so it is
...
def build_router(container: "AppContainer") -> APIRouter:
    ...
    run_repository = RecipeRunRepository()
    ...
    def _readiness_aggregator():
        if "mgr" not in _readiness:
            _readiness["mgr"] = build_readiness_aggregator(container)
    ...
    async def get_readiness(
        ...
        caller's role. `recipe_id` narrows the content facet to one recipe."""
        return await _readiness_aggregator().evaluate(current_user, recipe_id=recipe_id)

    # --- the wizard's recipe run (admin-only) -----------------------------

    @setup_router.post(
        "/runs",
        response_model=RecipeRunView,
        status_code=201,
        summary="Start (or return the active) onboarding run",
    )
    async def create_setup_run(
        body: CreateRecipeRunRequest,
        current_user: User = Depends(get_current_active_user),
    ) -> RecipeRunView:
        ...
        # Drive it forward in the background (see `RecipeRunner.
        ...
        return run_view(runner, _recipe_catalog(), run)

    @setup_router.get(
        "/runs/active",
        response_model=RecipeRunView,
        summary="The currently active onboarding run, if any",
    )
    ...
    @setup_router.get(
        "/runs/{run_id}",
        response_model=RecipeRunView,
        summary="Durable onboarding-run detail",
    )
    ...
    @setup_router.post(
        ...
        response_model=RecipeRunView,
        summary="Apply a run action (pause|resume|cancel|retry_step|grant_consent)",
    )
    async def act_on_setup_run(
        run_id: str,
        action: str,
        body: Optional[RecipeRunActionRequest] = None,
        current_user: User = Depends(get_current_active_user),
    ) -> RecipeRunView:
        _require_admin_or_404(current_user)
        runner = _runner()
    ...
    @setup_router.get(
        "/recipes",
        summary="Recipes the wizard can offer",
    )
