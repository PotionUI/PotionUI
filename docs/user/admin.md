...
A **Native** configuration (a model running in-process rather than through Ollama/OpenRouter) has an optional **thinking mode**: **model default** (leave the loaded chat template's own behavior alone), **enabled**, or **disabled**. It only takes effect for a checkpoint whose loaded chat template actually exposes an explicit thinking/reasoning switch — the Qwen3 family, for example — and every reply reports which mode actually applied, so requesting a mode a checkpoint doesn't support is never silently ignored. Until this gets its own control in the configuration form, set it by adding `"thinking": true` (or `false`) to the configuration's `provider_options`.
