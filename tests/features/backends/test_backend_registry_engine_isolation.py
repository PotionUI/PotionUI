"""
Each in-process backend must execute on its OWN GenerationEngine.

Backends run in parallel (one generation per backend), and GenerationEngine
...
def _registry_without_db(factory) -> BackendRegistry:
    ...
    registry.generation_engine_factory = factory
...
class TestBackendGetsItsOwnGenerationEngine(unittest.TestCase):
    def test_each_backend_instance_gets_a_distinct_manager(self):
        registry = _registry_without_db(lambda: Mock(name="generation_engine"))
        ...
        self.assertIsNotNone(a.generation_engine)
        self.assertIsNotNone(b.generation_engine)
        self.assertIsNot(
            a.generation_engine,
            b.generation_engine,
            "backends share a GenerationEngine: cancelling one would abort the other",
        )
