...
def repos_on_test_db(mock_db):
    ...
    repositories bound their own `db` name at import time, so without this they
    would keep writing to the real one (PersistenceTestBase redirects the same
    two modules for the same reason).
    """
