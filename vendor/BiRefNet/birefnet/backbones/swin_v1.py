...
# Source file: models/backbones/swin_v1.py at commit
# 25cb9309bacf3dde954e4584594e16e142c51de5.
# License: MIT (see LICENSE in this directory). Copyright (c) 2024 ZhengPeng.
# Local modifications: gradient checkpointing (`use_checkpoint` and its
# `torch.utils.checkpoint` import) and the `frozen_stages` / `_freeze_stages` /
# `train()` machinery are removed - all three exist only to shape a training
# run. The forward pass, the module layout and the key space are unchanged, so
# these checkpoints load under their own names.
