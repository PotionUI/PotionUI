...
MediaManager over a real FilePathResolver. Only the settings manager and the
...
@pytest.fixture
def saved_mesh(repos_on_test_db, settings_manager, minimal_glb_file):
...
class TestMeshServing:
    ...
    def test_final_mesh_is_served_with_the_gltf_media_type(self, media_manager, saved_mesh):
    ...
    def test_temporary_mesh_is_served_from_the_tmp_route(
        self, repos_on_test_db, media_manager, settings_manager, minimal_glb_file
    ):
        handler = MeshGenerationOutputHandler(generate_ulid(), None, settings_manager)
    ...
    def test_thumbnail_request_is_not_available(self, media_manager, saved_mesh):
    ...
    def test_unknown_mesh_filename_is_not_found(self, media_manager, saved_mesh):
        gen_id, _filename, _bytes = saved_mesh

        with pytest.raises(ValueError):
            media_manager.get_generation_media(gen_id, 'nope.glb')
...
class TestMeshPathContainment:
    ...
    def test_traversal_never_returns_the_planted_bytes(self, media_manager, planted_secret):
        ...
        served = None
        try:
            served = media_manager.get_temp_media("../../secret.glb")
        except ValueError:
            pass
