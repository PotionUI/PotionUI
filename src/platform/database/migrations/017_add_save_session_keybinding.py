"""017 adds the `save_session` default keybinding (key `s`, "Save Session")
to `keybinding_defaults`.
...
def up():
    with db.get_cursor() as cursor:
        cursor.execute(
            ...
            VALUES (?, 's', '', 'Save Session', 'generation', 'generate',
                    'Save the current session, or open Save As if none is selected', 1, 'system', 18)
            """,
            (_ACTION_ID,),
        )
    print(
        f"Migration 017_add_save_session_keybinding: seeded "
...
def down():
    ...
    print(
        f"Migration 017_add_save_session_keybinding: removed "
        f"'{_ACTION_ID}' default keybinding"
    )
