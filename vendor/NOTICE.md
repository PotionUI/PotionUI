...
| `src/platform/runtime/native/arch/yue2/{tokenizer,detect}.py` | `multimodal-art-projection/YuE` `src/yue2/tokenization_yue2.py` / `modeling_yue2.py` | Apache-2.0 | The special-token id table (`tiktoken.Encoding`'s special-token construction, ported as a plain dict) and the checkpoint's own module key names used as detection signatures. The base BPE vocabulary is not re-vendored: it is verified byte-identical to the already-bundled `text_encoders/assets/qwen3_tokenizer` asset (see `tokenizer.py`'s module docstring), so `tokenizer.py` wraps that shared asset instead of shipping YuE2's `qwen.tiktoken` a second time. |
| `src/platform/runtime/native/arch/yue2/vae.py` | `multimodal-art-projection/YuE` `src/yue2/modeling_vae.py` (itself "derived from stable-audio-tools `a6ae0cdf8b2eb1567a4b42ceadddec3712d99d45`, Copyright (c) 2023 Stability AI; Copyright (c) 2022 NVIDIA CORPORATION" per that file's own header — the YuE repository as a whole is Apache-2.0, but this specific module is MIT-derived) | MIT | The Oobleck decoder architecture (residual units, up/down-sample blocks, layer ordering) and the SnakeBeta activation. Decode-only (v1 does not port the encoder); weight-norm folded at load time via the existing `vae/minimax_music3_dav.py` helper rather than re-implemented. |
