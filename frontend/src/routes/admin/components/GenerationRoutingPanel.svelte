...
<DetailSection label="Routing" padded={false}>
	{#snippet headerExtra()}
		<Badge variant="signal" size="sm" class="font-mono">{routing.chosen.backend_name}</Badge>
	{/snippet}
	...
	<div class="px-4 sm:px-5 py-4 space-y-3">
		<div>
			...
			{#if routing.chosen.reason}
				<p class="text-xs text-fg-muted leading-relaxed mt-1">{routing.chosen.reason}</p>
			{/if}
		</div>

		{#if otherCandidates.length > 0}
			<div>
				<p class="text-2xs font-mono uppercase tracking-[0.07em] text-fg-subtle mb-1.5">Other candidates</p>
				<div class="divide-y divide-line border border-line rounded">
					{#each otherCandidates as candidate (candidate.backend_id)}
						<div
							class="flex items-start justify-between gap-3 px-2.5 py-1.5 {candidate.dropped
								? 'text-fg-subtle'
								: 'text-fg-muted'}"
						>
	...
	{#if routing.rule_trace.length > 0}
		<button
			type="button"
			class="w-full flex items-center justify-between gap-3 px-4 sm:px-5 py-2.5 text-left border-t border-line hover:bg-surface-2/40 transition-colors duration-100"
		...
		{#if expanded}
			<div class="px-4 sm:px-5 py-3 overflow-x-auto border-t border-line bg-surface-2/40">
