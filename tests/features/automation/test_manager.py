"""Tests for AutomationManager (src/core/automation/manager.py)."""

import unittest
from typing import Dict, List, Optional

from src.features.automation.context import AutomationServices
from src.features.automation.manager import AutomationManager, GraphValidationError
...
class TestValidateGraph(unittest.TestCase):
    ...
    def setUp(self):
        self.manager = AutomationManager(repository=FakeRepository(), engine=FakeEngine(), registry=_registry_with_builtins())
...
class TestValidateGraphFilesystemDirectory(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.manager = AutomationManager(repository=FakeRepository(), engine=FakeEngine(), registry=registry)
...
class TestCreateRaisesOnInvalidGraph(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_create_raises_graph_validation_error(self):
        manager = AutomationManager(repository=FakeRepository(), engine=FakeEngine(), registry=_registry_with_builtins())
...
class TestTriggerLifecycle(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_start_all_enabled_starts_manual_trigger_and_stop_all_stops_it(self):
        ...
        engine = FakeEngine()
        manager = AutomationManager(repository=repository, engine=engine, registry=_registry_with_builtins())
