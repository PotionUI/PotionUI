"""record_completion tests: preset-name resolution at write time and
pass-through reads. The repository is mocked -- SQL correctness is covered
by test_generation_stats_repository.py.
"""
...
class TestRecordCompletion:
    def test_resolves_preset_name_from_disk(self, mock_repo, mock_file_preset_repository):
        template = Mock()
        template.name = "SDXL Base (Realistic)"
        mock_file_preset_repository.find_preset_by_id.return_value = template

        record_completion(
            mock_repo, mock_file_preset_repository,
    ...
    def test_falls_back_to_preset_id_when_preset_deleted_from_disk(
        self, mock_repo, mock_file_preset_repository
    ):
    ...
    def test_no_preset_id_skips_lookup(self, mock_repo, mock_file_preset_repository):
        record_completion(
            mock_repo, mock_file_preset_repository,
    ...
    def test_repository_failure_is_swallowed(self, mock_repo, mock_file_preset_repository):
        ...
        which also wraps its own call in a try/except, but this function's own
    ...
    # to GenerationStatsRepository - there is no read-side operation to cover.
