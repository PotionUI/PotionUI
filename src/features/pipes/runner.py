...
class PipeInstallRunner:
    def __init__(self, pipe_catalog, installer: PipeInstaller, connection_hub=None):
        self.pipe_catalog = pipe_catalog
        self.installer = installer
        self.connection_hub = connection_hub or admin_connection_hub
    ...
    async def _broadcast(self, pipe_name: str, status: str, message: Optional[str]) -> None:
        try:
            await self.connection_hub.broadcast({
                "type": MESSAGE_TYPE,
                "pipe": pipe_name,
                "status": status,
                "message": message,
