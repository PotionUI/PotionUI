"""Unit tests for the ZImageClipTextEncoder ABC adapter.
...
path) and pack its role-keyed dicts into a ConditioningModel. Z-Image uses
TRUE CFG, so the negative pass is always encoded when CFG is requested
(mirrors QwenClipTextEncoder / AnimaClipTextEncoder).

This adapter used to call ``self.encoder.encode_weighted(...)``
directly, so the Qwen3-4B forward always ran on the CPU in fp32 -- it never
...
``run_text_encode`` name on ``z_image_clip`` to monkeypatch, so the
...
class _FakeEncoder(NativeTextEncoder):
    ...
    Mirrors the Z-Image output contract: ``{"context", "attention_mask"}`` and
    NO pooled vector (single Qwen3-4B text encoder, no CLIP-L).
    """
...
def _adapter(fingerprint=None, device="cpu"):
    ...
    fake = _FakeEncoder()
    enc = ZImageClipTextEncoder(fake, device=device, model_fingerprint=fingerprint)
    return enc, fake
...
def test_empty_negative_still_encoded_for_true_cfg():
    # Z-Image's true-CFG sampler needs the uncond pass even when the negative
...
def test_model_fingerprint_surfaced_for_prompt_encoder_cache():
    enc, _ = _adapter(fingerprint="zimage|te|dit|lora=none")
    assert enc._model_fingerprint == "zimage|te|dit|lora=none"
...
def test_routes_through_run_text_encode(monkeypatch):
    ...
    ``self.encoder.encode_weighted`` -- otherwise the Qwen3-4B forward never
    leaves the CPU."""
    enc, fake = _adapter(fingerprint="zimage|te|dit")
