...
- `source` is a Jinja2 template evaluated with the same `form`/`generation`/`preset`/`paths`
  context as everywhere else in `pipeline.yml` (see "Template contexts" above), or the literal
...
version; see "The `@loop` recipe" above for `@loop` mechanics in general) and change only the
...
"Configuration (admin-set)" above) — SDXL declares `checkpoint_tags` and `lora_tags`, then each
