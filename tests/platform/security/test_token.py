...
class TestTokenManager:
    ...
    def token_manager(self, mock_config):
        """Create a TokenManager with mock config."""
        return TokenManager(mock_config)

    def test_create_access_token_default_expiry(self, token_manager, mock_config):
    ...
    def test_create_access_token_custom_expiry(self, token_manager, mock_config):
        """Test token creation with custom expiry."""
        data = {"sub": "testuser", "user_id": "test-123"}
        custom_delta = timedelta(hours=2)

        token = token_manager.create_access_token(data, custom_delta)
    ...
    def test_decode_token_valid(self, token_manager):
    ...
    def test_decode_token_invalid(self, token_manager):
        ...
        result = token_manager.decode_token("invalid.token.here")

        assert result is None

    def test_decode_token_expired(self, token_manager, mock_config):
        """Test decoding an expired token."""
        data = {"sub": "testuser", "user_id": "test-123"}
        expired_delta = timedelta(seconds=-1)  # Already expired

        token = token_manager.create_access_token(data, expired_delta)
        result = token_manager.decode_token(token)

        assert result is None

    def test_decode_token_missing_username(self, token_manager, mock_config):
    ...
    def test_decode_token_missing_user_id(self, token_manager, mock_config):
    ...
    def test_token_preserves_additional_claims(self, token_manager, mock_config):
