...
// content/plugins/node_modules/svelte/src/internal/flags/legacy.js
...
// content/plugins/node_modules/svelte/src/internal/client/dom/legacy/lifecycle.js
...
// content/plugins/marketplace/example-field/frontend/src/ExampleStarsField.svelte
var root = from_html(`<label class="example-stars-label svelte-1mgfg58"> </label>`);
var root_1 = from_html(`<button type="button" role="radio">&#9733;</button>`);
var root_2 = from_html(`<p class="example-stars-desc svelte-1mgfg58"> </p>`);
var root_3 = from_html(`<div class="example-stars-field svelte-1mgfg58"><!> <div class="example-stars-row svelte-1mgfg58" role="radiogroup"></div> <!></div>`);
var $$css = {
  hash: "svelte-1mgfg58",
  code: ".example-stars-field.svelte-1mgfg58 {padding:0.5rem 0;}.example-stars-label.svelte-1mgfg58 {display:block;font-size:0.75rem;font-weight:500;margin-bottom:0.375rem;opacity:0.8;}.example-stars-row.svelte-1mgfg58 {display:flex;gap:0.25rem;}.example-star.svelte-1mgfg58 {background:none;border:none;font-size:1.5rem;line-height:1;cursor:pointer;opacity:0.3;padding:0;transition:opacity 0.15s ease, transform 0.1s ease;}.example-star.svelte-1mgfg58:hover {transform:scale(1.15);}.example-star.filled.svelte-1mgfg58 {opacity:1;}.example-stars-desc.svelte-1mgfg58 {font-size:0.75rem;opacity:0.6;margin-top:0.25rem;}"
};
function ExampleStarsField($$anchor, $$props) {
  if (new.target) return createClassComponent({ component: ExampleStarsField, ...$$anchor });
  push($$props, false);
  append_styles($$anchor, $$css);
  const label = mutable_source();
  const description = mutable_source();
  const max = mutable_source();
  const rating = mutable_source();
  ...
  let host = prop($$props, "host", 12, void 0);
  function setRating(star) {
    if (name()) onChange()(name(), star);
  }
  legacy_pre_effect(() => (deep_read_state(config()), deep_read_state(name())), () => {
    set(label, config().title || name() || "");
  });
  legacy_pre_effect(() => deep_read_state(config()), () => {
    set(description, config().description || "");
  });
  legacy_pre_effect(() => deep_read_state(config()), () => {
    set(max, config().configuration?.max_stars || 5);
  });
  legacy_pre_effect(() => deep_read_state(value()), () => {
    set(rating, typeof value() === "number" ? value() : 0);
  });
  ...
  var node = child(div);
  {
    var consequent = ($$anchor2) => {
      ...
      reset(label_1);
      template_effect(() => set_text(text2, get(label)));
  ...
  each(div_1, 5, () => (get(max), untrack(() => Array(get(max)))), index, ($$anchor2, _, i) => {
    const star = derived_safe_equal(() => i + 1);
    var button = root_1();
    let classes;
    template_effect(() => {
      classes = set_class(button, 1, "example-star svelte-1mgfg58", null, classes, { filled: get(star) <= get(rating) });
      set_attribute2(button, "aria-checked", get(star) === get(rating));
      set_attribute2(button, "title", `${get(star) ?? ""} star${get(star) !== 1 ? "s" : ""}`);
    });
    event("click", button, () => setRating(get(star)));
  ...
  reset(div);
  template_effect(() => set_attribute2(div_1, "aria-label", get(label) || "Rating"));
  append($$anchor, div);
  return pop($$exports);
}
export {
  ExampleStarsField as default
};
//# sourceMappingURL=ExampleStarsField.js.map
