...
function ImportWorkflowTab($$anchor, $$props) {
  ...
  let loraAllKept = user_derived(() => get(loraChainNodes).length > 0 && (get(loraConverted) ? get(loraReplacedIds).size === 0 : get(loraChainNodes).every((n) => get(loraKeepFixed).has(n.node_id))));
  let modelChainInfo = user_derived(() => get(analysis)?.model_chain || null);
  ...
  function nodeClassLabel(nodeId) {
    ...
    const candidate = (get(analysis)?.candidates || []).find((c) => c.node_id === nodeId);
    if (candidate?.class_type) return candidate.class_type;
    const raw = get(workflowJson) && typeof get(workflowJson) === "object" ? get(workflowJson)[nodeId] : null;
    if (raw && typeof raw === "object" && raw.class_type) return raw.class_type;
    return nodeId;
  }
  let modelChainInsertionLabel = user_derived(() => get(modelChainInfo) ? `${nodeClassLabel(get(modelChainInfo).target_node_id)}.${get(modelChainInfo).target_input}` : "");
  ...
  function applyLoraPickerConversion(tab, keepFixedIds, fieldNameHint) {
    ...
    if (!tab || get(loraConverted)) return false;
    if (!nodes.length && !get(modelChainInfo)) return false;
  ...
  function addLoraPickerNoChain() {
    applyLoraPickerConversion(get(activeTab) || get(form).tabs[0], /* @__PURE__ */ new Set(), "loras");
  }
  user_effect(() => {
  ...
  function configForTypeChange(item, newType) {
    ...
    if (newType === "model") {
      ...
      return {
        ...
        config: {
          model_type: guessModelTypeFromInputName(inputName),
          allow_info_modal: true
        }
      };
    }
