...
from src.platform.security.token import TokenManager
...
class TestAuthManager:
    ...
    def mock_claim_tokens(self):
        """Create a mock ClaimTokenManager. Default: no valid token."""
        from src.platform.security.claim_token import ClaimTokenManager
        tokens = Mock(spec=ClaimTokenManager)
    ...
    def auth_manager(
        ...
        """Create an AuthManager with mocks."""
        return AuthManager(
    ...
    def test_register_success_first_user_admin(self, auth_manager, mock_user_repository, mock_instance_claim, mock_claim_tokens):
        ...
        user, token = auth_manager.register(
            username="newuser",
            email="newuser@example.com",
    ...
    def test_register_success_subsequent_user(self, auth_manager, mock_user_repository, mock_instance_claim, mock_settings_manager, mock_claim_tokens):
        ...
        mock_settings_manager.get_setting.return_value = "open"
    ...
    def test_register_closed_after_claim(self, auth_manager, mock_user_repository, mock_instance_claim, mock_settings_manager):
        """Once claimed, a closed registration_policy rejects new accounts."""
        mock_user_repository.exists_by_username.return_value = False
        mock_user_repository.exists_by_email.return_value = False
        mock_instance_claim.is_claimed.return_value = True
        mock_settings_manager.get_setting.return_value = "closed"

        with pytest.raises(ValueError) as exc_info:
            auth_manager.register("newuser", "new@example.com", "password123")

        assert "already has an owner" in str(exc_info.value)
        mock_user_repository.create_claiming_instance.assert_not_called()

    def test_register_reopened_after_claim(self, auth_manager, mock_user_repository, mock_instance_claim, mock_settings_manager):
        ...
        user, _ = auth_manager.register("newuser", "newuser@example.com", "password123")
        assert user.account_type == AccountType.USER

    def test_register_remote_unclaimed_requires_token(self, auth_manager, mock_user_repository, mock_instance_claim, mock_claim_tokens):
    ...
    def test_register_remote_unclaimed_with_valid_token(self, auth_manager, mock_user_repository, mock_instance_claim, mock_claim_tokens):
        ...
        user, _ = auth_manager.register(
            "newuser", "newuser@example.com", "password123",
            origin_is_loopback=False, claim_token="right",
        )
        assert user.account_type == AccountType.ADMIN

    def test_register_loopback_unclaimed_needs_no_token(self, auth_manager, mock_user_repository, mock_instance_claim, mock_claim_tokens):
    ...
    def test_register_username_exists(self, auth_manager, mock_user_repository):
        """Test registration fails when username exists."""
        mock_user_repository.exists_by_username.return_value = True

        with pytest.raises(ValueError) as exc_info:
            auth_manager.register("existing", "new@example.com", "password")

        assert "Username already exists" in str(exc_info.value)

    def test_register_email_exists(self, auth_manager, mock_user_repository):
        """Test registration fails when email exists."""
        mock_user_repository.exists_by_username.return_value = False
        mock_user_repository.exists_by_email.return_value = True

        with pytest.raises(ValueError) as exc_info:
            auth_manager.register("newuser", "existing@example.com", "password")

        assert "Email already exists" in str(exc_info.value)

    def test_register_executes_hooks(self, auth_manager, mock_plugin_registry, mock_user_repository, mock_instance_claim):
        ...
        auth_manager.register("newuser", "newuser@example.com", "password")
    ...
    def test_register_blocked_by_hook(self, auth_manager, mock_plugin_registry):
        ...
        with pytest.raises(ValueError) as exc_info:
            auth_manager.register("newuser", "new@example.com", "password")

        assert "Registration disabled" in str(exc_info.value)

    # Authentication tests

    def test_authenticate_success(self, auth_manager, mock_user_repository, mock_token_manager, sample_user):
        ...
        user, token = auth_manager.authenticate("testuser", "password")
    ...
    def test_authenticate_with_remember_me(self, auth_manager, mock_user_repository, mock_token_manager, mock_auth_config, sample_user):
        ...
        user, token = auth_manager.authenticate("testuser", "password", remember_me=True)
    ...
    def test_authenticate_without_remember_me_uses_default_expiry(self, auth_manager, mock_user_repository, mock_token_manager, sample_user):
        """Test authenticate without remember_me passes expires_delta=None."""
        mock_user_repository.get_by_username.return_value = sample_user
        mock_user_repository.update_last_login.return_value = sample_user

        user, token = auth_manager.authenticate("testuser", "password", remember_me=False)
    ...
    def test_authenticate_user_not_found(self, auth_manager, mock_user_repository):
    ...
    def test_authenticate_wrong_password(self, auth_manager, mock_user_repository, mock_password_hasher, sample_user):
    ...
    def test_authenticate_executes_hooks(self, auth_manager, mock_plugin_registry, mock_user_repository, sample_user):
        """Test that authentication executes before and after hooks."""
        mock_user_repository.get_by_username.return_value = sample_user
        mock_user_repository.update_last_login.return_value = sample_user

        auth_manager.authenticate("testuser", "password")
    ...
    def test_change_password_success(self, auth_manager, mock_user_repository, mock_password_hasher, sample_user):
    ...
    def test_change_password_wrong_current_password(self, auth_manager, mock_user_repository, mock_password_hasher, sample_user):
    ...
    def test_change_password_user_not_found(self, auth_manager, mock_user_repository):
    ...
    def test_authenticate_blocked_by_hook(self, auth_manager, mock_plugin_registry):
    ...
    def test_get_user_from_token_valid(self, auth_manager, mock_user_repository, sample_user):
        """Test getting user from valid token."""
        mock_user_repository.get_by_id.return_value = sample_user

        result = auth_manager.get_user_from_token("valid.jwt.token")

        assert result == sample_user

    def test_get_user_from_token_invalid(self, auth_manager, mock_token_manager):
        """Test getting user from invalid token."""
        mock_token_manager.decode_token.return_value = None

        result = auth_manager.get_user_from_token("invalid.token")

        assert result is None

    def test_get_user_from_token_user_not_found(self, auth_manager, mock_user_repository):
        """Test getting user from token when user doesn't exist."""
        mock_user_repository.get_by_id.return_value = None

        result = auth_manager.get_user_from_token("valid.token")

        assert result is None

    # WebSocket authentication tests

    def test_authenticate_websocket_no_token(self, auth_manager):
        """Test WebSocket auth with no token."""
        user, error = auth_manager.authenticate_websocket(None)

        assert user is None
        assert error == "Authentication required"

    def test_authenticate_websocket_invalid_token(self, auth_manager, mock_token_manager):
        """Test WebSocket auth with invalid token."""
        mock_token_manager.decode_token.return_value = None

        user, error = auth_manager.authenticate_websocket("invalid.token")

        assert user is None
        assert error == "Invalid token"

    def test_authenticate_websocket_user_not_found(self, auth_manager, mock_user_repository):
        """Test WebSocket auth when user not found."""
        mock_user_repository.get_by_id.return_value = None

        user, error = auth_manager.authenticate_websocket("valid.token")

        assert user is None
        assert error == "User not found"

    def test_authenticate_websocket_success(self, auth_manager, mock_user_repository, sample_user):
