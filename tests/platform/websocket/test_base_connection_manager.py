"""Tests for BaseConnectionManager and its use by the admin/download managers.
...
from src.platform.websocket.admin_connection_manager import AdminConnectionManager
...
class TestAdminConnectionManagerInheritsBase:
    """Admin has no logic of its own left - these confirm inheritance wires
    connect/broadcast through, not just that AdminConnectionManager exists."""
...
class TestDownloadConnectionManagerOverrides:
    ...
    async def test_disconnect_clears_download_subscriptions(self):
        manager = DownloadConnectionManager()
        websocket = AsyncMock(spec=WebSocket)
