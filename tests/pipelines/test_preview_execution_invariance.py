...
(PresetManager.get_pipeline) and real generation execution
...
hands it to both GenerationOrchestrator and PresetManager) and asserts the
graph's node configuration/order matches the pipes execution would receive.
"""
...
@pytest.fixture
def preset_manager(shared_pipeline_builder):
...
def test_graph_pipes_match_execution_pipes(
    ...
    preset_manager.file_repo.find_preset_by_id.return_value = preset_template
    ...
    # "Preview" path: through PresetManager.get_pipeline -> build_graph.
    preset_manager.pipe_catalog.get_pipe.return_value = None  # status irrelevant here
    graph = preset_manager.get_pipeline('test-preset', mode, form_data)
