"""
System monitoring manager - coordinates system monitoring operations.

This module provides the SystemMonitorManager class that coordinates system
...
from src.features.system_monitor.connection_manager import MonitoringConnectionManager, WebSocketProtocol
...
class SystemMonitorManager:
    ...
    def __init__(
        ...
        """
        Initialize the SystemMonitorManager.

        Args:
            system_monitor: SystemMonitor instance for collecting stats
            gpu_manager: Optional GpuManager for GPU-specific operations
            plugin_registry: Optional PluginRegistry for hook execution
        """
        self.system_monitor = system_monitor
        self.gpu_manager = gpu_manager
        self.plugins = plugin_registry
        self.connection_manager = MonitoringConnectionManager()
    ...
    async def _monitor_system(self) -> None:
        ...
        while self.connection_manager.has_connections():
            try:
                # Get system stats
                stats = self.get_system_stats()

                # Broadcast to all connected clients
                await self.connection_manager.broadcast(stats)
