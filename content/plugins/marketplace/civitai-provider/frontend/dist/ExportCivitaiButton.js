...
// content/plugins/marketplace/civitai-provider/frontend/src/ExportCivitaiButton.svelte
var root = from_html(`<button class="bg-black/50 hover:bg-black/70 disabled:opacity-50 text-white p-3 rounded-lg shadow-lg backdrop-blur-sm transition-colors" title="Export for Civitai"><svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path></svg></button>`);
function ExportCivitaiButton($$anchor, $$props) {
  if (new.target) return createClassComponent({ component: ExportCivitaiButton, ...$$anchor });
  push($$props, false);
  const isImage = mutable_source();
  let context = prop($$props, "context", 28, () => ({}));
  let hookName = prop($$props, "hookName", 12, "");
  let pluginId = prop($$props, "pluginId", 12, "");
  let exporting = mutable_source(false);
  ...
  async function handleExport() {
    if (get(exporting) || !context().generationId) return;
    set(exporting, true);
    try {
      ...
      const response = await fetch(`/api/plugins/civitai-provider/export-png?${params}`, { headers: token ? { Authorization: `Bearer ${token}` } : {} });
      if (!response.ok) {
  ...
  legacy_pre_effect(() => deep_read_state(context()), () => {
    set(isImage, (context().fileType || "").toLowerCase() === "image");
  });
  ...
  var node = first_child(fragment);
  {
    var consequent = ($$anchor2) => {
      var button = root();
      template_effect(() => button.disabled = get(exporting));
      event("click", button, handleExport);
      append($$anchor2, button);
    };
    if_block(node, ($$render) => {
      if (get(isImage)) $$render(consequent);
    });
  }
...
export {
  ExportCivitaiButton as default
};
//# sourceMappingURL=ExportCivitaiButton.js.map
