...
The Advanced tab's **Sampler** select now also offers `dpmpp_2m_sde`, `dpmpp_3m`, `res_multistep` and `er_sde` (the pipe already accepted all four; only Euler/DPM++ 2M/UniPC were exposed before). A **Schedule** select sits next to it: "Model default" (`""`, the shift-based mu selection the table above describes) or "Beta" (`schedule="beta"`, Beta-CDF-spaced sigmas via `build_sigmas` in `sampling/flow_schedule.py`). Beta spacing bypasses the mu/shift math outright — it ignores `mu_schedule`/`fixed_mu`/`dynamic_shift` entirely, so pairing it with the Base profile has no extra effect beyond Base's own CFG change. Civitai users porting a recipe typically pair `er_sde` + Beta for photographic skin and fabric texture at the cost of losing the turbo checkpoint's tuned schedule; `euler` + Model default stays the fastest, most-tested combination and is the right default outside a specific recipe.
