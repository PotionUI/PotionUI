...
| Wan 2.1 FLF2V (CLIP-vision `img_emb`, in_dim 36) | N/A | Rejected — `generator/img2vid_wan22` detects `img_emb.emb_pos` and raises (model-free pipeline test; no GPU validation) | Rejected, same guard | **Unsupported / unverified** — `chain_video_wan22`'s i2v-set guard only checks in_dim (== 36), which this checkpoint also has, and has no `img_emb`/`emb_pos` check; a Director i2v/chain shot loading this checkpoint is not rejected today and its actual behavior through `chain_video_wan22`'s own concat construction is unverified. A dedicated guard there is a separate, not-yet-landed fix (tracked as INF-W03) — not implemented by this change. |
