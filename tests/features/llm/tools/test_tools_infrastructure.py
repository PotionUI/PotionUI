...
class TestToolContext:
    def test_default_values(self):
        ...
        assert ctx.segment_manager is None
        assert ctx.model_index_manager is None
        assert ctx.preset_manager is None
        assert ctx.phrasebook_manager is None
...
class TestToolExecutorRescue:
    ...
    @pytest.mark.asyncio
    async def test_local_models_malformed_video_director_call_gets_a_corrective_nudge(self):
        """The verbatim shape a local model produced: a pseudo-XML
        tag whose `operations` payload is a Python repr AND an invalid dict
        literal. Unrecoverable, so it must steer the model rather than dispatch
        a string of operations or drop the call silently."""
        from src.features.llm.tools.builtin.video_director_tool import UpdateVideoDirectorTool

        executor, llm_service = self._make_executor(UpdateVideoDirectorTool())
        malformed = (
        ...
        llm_service.generate_with_tools.side_effect = [
            make_llm_response(f"Composing that now.\n{malformed}", tool_calls=[]),
            make_llm_response("Updated the shots."),
        ]

        response, executions = await executor.execute_with_tools(
            messages=[{"role": "user", "content": "make me a three shot video"}],
            llm_id="model-1",
            system_message="sys",
            tool_context=make_context(),
            allowed_tools=["update_video_director"],
        )
        ...
        nudge = [
            m["content"]
            for m in llm_service.generate_with_tools.call_args_list[1].kwargs["messages"]
            if m["role"] == "system" and "<tool_call>" in m["content"]
        ]
        assert len(nudge) == 1
        assert "two colons" in nudge[0]
        assert '{"op": "set_prompt", "prompt": "text"}' in nudge[0]

    @pytest.mark.asyncio
    async def test_token_mangled_unquoted_attribute_call_is_dispatched(self):
        """The second live sample's transport: an unquoted `operations=[...]`
        attribute, quote characters wrapped in special-token delimiters, and
        unquoted object keys. Complete, so it must reach the tool as real
        operations rather than being steered."""
        from src.features.llm.tools.builtin.video_director_tool import UpdateVideoDirectorTool
        ...
        llm_service.generate_with_tools.side_effect = [
            make_llm_response(mangled, tool_calls=[]),
            make_llm_response("Added the shot."),
        ]

        response, executions = await executor.execute_with_tools(
            messages=[{"role": "user", "content": "add a three second dune shot"}],
            llm_id="model-1",
            system_message="sys",
            tool_context=make_context(),
            allowed_tools=["update_video_director"],
        ...
        assert [te.tool_name for te in executions] == ["update_video_director"]
        assert executions[0].arguments == {
            "operations": [
        ...
        assert "tool_action" not in (response.content or "")
