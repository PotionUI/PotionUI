"""
Tests for SystemMonitorCoordinator.
...
from src.features.system_monitor.connection_hub import MonitoringConnectionHub
from src.platform.plugins.hooks import HookContext
from src.features.system_monitor.hooks import SYSTEM_MONITOR_HOOKS


class TestSystemMonitorCoordinator:
    """Tests for SystemMonitorCoordinator class."""
    ...
    @pytest.fixture
    def manager(self, mock_system_monitor, mock_gpu_monitor, mock_plugin_registry):
        """Create a SystemMonitorCoordinator with mocked dependencies."""
        return SystemMonitorCoordinator(
    ...
    @pytest.fixture
    def manager_no_plugins(self, mock_system_monitor, mock_gpu_monitor):
        """Create a SystemMonitorCoordinator without plugin registry."""
        return SystemMonitorCoordinator(
            system_monitor=mock_system_monitor,
            gpu_monitor=mock_gpu_monitor,
    ...
    def test_connection_hub_initialized(self, manager):
        """Test that connection manager is initialized."""
        assert manager.connection_hub is not None
        assert isinstance(manager.connection_hub, MonitoringConnectionHub)
...
class TestMonitoringConnectionHub:
    """Tests for MonitoringConnectionHub class."""

    @pytest.fixture
    def connection_hub(self):
        """Create a MonitoringConnectionHub."""
        return MonitoringConnectionHub()

    def test_add_connection(self, connection_hub):
        """Test adding a connection."""
        mock_ws = Mock()
        connection_hub.add_connection(mock_ws)

        assert connection_hub.connection_count() == 1
        assert connection_hub.has_connections() is True

    def test_remove_connection(self, connection_hub):
        """Test removing a connection."""
        mock_ws = Mock()
        connection_hub.add_connection(mock_ws)
        connection_hub.remove_connection(mock_ws)

        assert connection_hub.connection_count() == 0
        assert connection_hub.has_connections() is False

    def test_remove_nonexistent_connection(self, connection_hub):
    ...
    @pytest.mark.asyncio
    async def test_broadcast_to_connections(self, connection_hub):
        """Test broadcasting message to all connections."""
        mock_ws1 = AsyncMock()
        mock_ws2 = AsyncMock()

        connection_hub.add_connection(mock_ws1)
        connection_hub.add_connection(mock_ws2)

        message = {"test": "data"}
        await connection_hub.broadcast(message)

        mock_ws1.send_text.assert_called_once()
        mock_ws2.send_text.assert_called_once()

    @pytest.mark.asyncio
    async def test_broadcast_removes_failed_connections(self, connection_hub):
        ...
        failed = await connection_hub.broadcast({"test": "data"})

        assert mock_ws2 in failed
        assert connection_hub.connection_count() == 1

    @pytest.mark.asyncio
    async def test_broadcast_no_connections(self, connection_hub):
        """Test broadcasting with no connections returns empty list."""
        result = await connection_hub.broadcast({"test": "data"})
        assert result == []


class TestSystemMonitorCoordinatorAsync:
    """Async tests for SystemMonitorCoordinator."""
    ...
    def manager(self, mock_system_monitor):
        """Create a SystemMonitorCoordinator."""
        return SystemMonitorCoordinator(
            system_monitor=mock_system_monitor,
            gpu_monitor=None,
            plugin_registry=None
