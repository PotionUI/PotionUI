...
Classic Wan i2v checkpoints (with or without the FLF `emb_pos` table) share `in_dim=36` with the modern concat-i2v contract but condition through a CLIP-vision `img_emb` cross-attention path (`arch/wan/model.py`'s `MLPProj`) that neither Wan generator ever feeds `clip_fea` into. A single shared helper, `src/pipelines/pipes/_shared/generation/wan_i2v_contract.py`'s `require_concat_i2v_contract`, checks the resolved module's own `img_emb` attribute (populated only for `model_type == "i2v"` checkpoints — channel count alone can't distinguish the two) and is called from both entry points: `generator/img2vid_wan22.build_context` (the high-noise expert) and `generator/chain_video_wan22.prepare_set` for the i2v set (every expert that can execute — high AND low, so an incompatible low expert beside a compatible high still rejects).
...
| Wan 2.1 classic i2v / FLF2V (CLIP-vision `img_emb`, in_dim 36, with or without the FLF `emb_pos` table) | N/A | **Rejected** — `generator/img2vid_wan22.build_context` calls `require_concat_i2v_contract` on the high-noise expert's module and raises before any VAE/concat work (model-free pipeline test: `test_classic_clip_vision_i2v_raises_before_any_vae_or_concat_work`); no GPU validation — real-weight behavior remains unverified | **Rejected**, same guard and same test (FLF parametrization) | **Rejected** — `generator/chain_video_wan22.prepare_set` calls the same shared helper on every expert of the i2v set (high and low) before any concat/denoise work (model-free pipeline tests: `test_i2v_set_classic_i2v_without_flf_raises_before_any_work`, `test_i2v_set_classic_flf2v_raises_before_any_work`, `test_i2v_set_incompatible_low_expert_raises_even_with_compatible_high`, `test_chain_continuation_needing_incompatible_i2v_set_raises_before_that_segments_concat`); no GPU validation — real-weight behavior remains unverified |
