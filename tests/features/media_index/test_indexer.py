...
from src.features.media_index.indexer import (
    MAX_ATTEMPTS,
    PASS_CLIP_EMBED,
    PASS_PROMPT_EMBED,
    PASS_TAGS,
...
class IndexerTestBase(MediaIndexTestBase):
    def _indexer(self, tagger=None, embedder=None, store=None, file_service=None,
...
class TestQueueFeeding(IndexerTestBase):
...
class TestProcessPending(IndexerTestBase):
    ...
    def test_failures_count_attempts_then_park_as_failed(self):
        ...
        manager = self._indexer(FakeTagger(error=RuntimeError("corrupt image")))
...
class TestRetagStale(IndexerTestBase):
    def test_model_switch_deletes_old_provenance_and_requeues(self):
        self._make_file("f1")
        self.repo.enqueue_files(["f1"], PASS_TAGS)
        old_manager = self._indexer(FakeTagger(provenance="model-old"))
        old_manager.process_pending(PASS_TAGS, batch_size=10)

        new_manager = self._indexer(FakeTagger(provenance="model-new"))
...
class TestStatus(IndexerTestBase):
...
class TestClipQueueFeeding(IndexerTestBase):
...
class ClipDrainTestBase(IndexerTestBase):
    def setUp(self):
        super().setUp()
        import tempfile
    ...
    def _clip_indexer(self, **kwargs):
        return self._indexer(file_service=FakeFileStore(base=self.storage_dir), **kwargs)
...
class TestClipEmbedPass(ClipDrainTestBase):
    ...
    def test_embed_failure_marks_batch_failed_with_attempts(self):
        ...
        manager = self._clip_indexer(embedder=FakeVisionEmbedder(error=RuntimeError("no space")))
...
class TestPromptEmbedPass(IndexerTestBase):
    ...
    def test_embed_failure_marks_batch_failed_with_attempts(self):
        ...
        manager = self._indexer(text_embedder=FakeTextEmbedder(error=RuntimeError("provider down")))
    ...
    def test_store_failure_fails_only_that_item(self):
        ...
        manager = self._indexer(prompt_store=real_store)
...
class TestGallerySearch(IndexerTestBase):
    def test_search_embeds_query_and_applies_relative_cutoff(self):
        ...
        manager = self._indexer(store=FakeGalleryStore(empty=False, hits=hits))
    ...
    def test_blank_query_returns_empty_without_embedding(self):
        manager = self._indexer(store=FakeGalleryStore(empty=False))
    ...
    def test_relative_cutoff_keeps_all_when_scores_are_close(self):
        ...
        assert MediaIndexer.apply_relative_cutoff(hits) == hits

    def test_relative_cutoff_handles_empty(self):
        assert MediaIndexer.apply_relative_cutoff([]) == []
    ...
    def test_embedder_switch_requeues_done_rows_on_search(self):
        ...
        manager = self._indexer(store=FakeGalleryStore(empty=True))
