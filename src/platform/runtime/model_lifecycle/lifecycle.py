"""
ModelLifecycle - central cache for models and other expensive-to-build
...
A generic key/fingerprint/loader cache that:
  ...
  - evicts LRU entries under HOST-RAM pressure on a miss (cached models sit
    ...
    GpuResidencyRegistry / placement planner at move_to() time, NOT here)
...
Thread-safe via RLock: the comfyui-backend plugin drives GenerationEngine
...
# GpuMonitor's VRAM safety margin but for host memory: a loader reads its
...
_default_lifecycle: Optional["ModelLifecycle"] = None
...
def get_model_lifecycle() -> Optional["ModelLifecycle"]:
    """Return the process-wide ModelLifecycle, or None if the app
...
class ModelLifecycle:
    ...
    def acquire(
        ...
        with self._lock:
            ...
            try:
                ...
                # GpuResidencyRegistry / placement planner at move_to() time.
        ...
        if value is not None:
            ...
            if refcount <= 2:
                ...
                try:
                    ...
                    get_residency_registry().mark_orphaned(value)
                except Exception:
    ...
    def begin_generation(self, owner: Optional[str]) -> None:
        ...
        worker thread), which the GenerationEngine does. The native queue is
        serial per backend (GenerationEngine._run_lock), so there is no concurrent
    ...
    def _cache_scope(self) -> str:
        ...
        if self.settings is None:
            return "preset"
        try:
            scope = self.settings.get_setting("model_cache_scope", "preset")
    ...
    def memory_policy(self) -> MemoryPolicy:
        ...
        if self.gpu_monitor is not None:
            try:
                vram_gb = self.gpu_monitor.get_vram_budget()
            except Exception as e:
    ...
    def leased_values(self) -> List[Any]:
        ...
        action's ``GpuResidencyRegistry.offload_all(device, exclude=...)`` —
    ...
    def entry_size_gb(self, key: str) -> Optional[float]:
        ...
        a size to ``GpuResidencyRegistry.note_resident``.
        """
        with self._lock:
            entry = self._entries.get(key)
    ...
    def cached_values(self) -> List[Any]:
        ...
        with ``GpuResidencyRegistry`` (a placement path that forgot to, now or
