"""Shared contract for recipe-run step executors.
...
`RecipeRunner.record_step_attempt`. Executors receive their collaborators
...
from src.features.recipes.records import RecipeRun
...
class StepContext:
    ...
    `report_progress` is the interim-progress seam (recipe-run T3.7 follow-up):
    ...
    row *before* the step finishes, so `GET /api/recipes/runs/{id}` (polled by
...
class StepResult:
    ...
    once handed to `RecipeRunner.record_step_attempt` - executors don't
    ...
    the run in `awaiting_consent` rather than advancing; `RecipeRunner.
    ...
    def to_safe_output(self) -> Dict[str, Any]:
        ...
        is no separate column for either) - `run_dto.RecipeStepAttemptView`
