...
class FakeModel:
    """Stand-in for a model wrapper with the shape ModelLifecycle knows
    ...
    (see ModelLifecycle's A3 refcount safety check) whenever something
...
class TestEvictionOrder:
    ...
    def test_cached_models_never_evicted_on_vram_style_budget(self, fake_gpu_monitor, monkeypatch):
        ...
        # under pressure from other processes). gpu_monitor present to prove
        # it is not consulted for admission.
        fake_gpu_monitor.get_vram_budget.return_value = 15.0
        manager = ModelLifecycle(gpu_monitor=fake_gpu_monitor, settings=None)
        gb = 1024 ** 3
        monkeypatch.setattr(
    ...
    def test_no_eviction_without_gpu_monitor(self):
        # Small estimates so the separate RAM-pressure check (independent of
        # gpu_monitor) has no reason to fire either - this test is only about
        # the VRAM-budget axis being a no-op without a gpu_monitor.
        manager = ModelLifecycle(gpu_monitor=None, settings=None)
        log = []
...
class TestCachedValuesAndLeasedValues:
    ...
    values a placement path forgot to register with GpuResidencyRegistry."""
...
class TestRamPressureEviction:
    ...
    Uses a manager with NO gpu_monitor (rather than the shared `manager`
...
class TestNeverEvictsKeyBeingAcquired:
    def test_recursive_acquire_for_different_key_is_not_evicted(self, fake_gpu_monitor):
        ...
        fake_gpu_monitor.get_vram_budget.return_value = 5.0
        manager = ModelLifecycle(gpu_monitor=fake_gpu_monitor, settings=None)
        log = []
...
class TestEntrySizeGb:
    ...
    GpuResidencyRegistry.note_resident) that needs the number without
