...
- LTX's token-conditioned generation (keyframes / IC-LoRA references) forces certain token positions to an exact clean value independent of guidance; `cfg_zero_star`'s rescale is a per-batch scalar that would otherwise perturb those positions at `cfg > 1`. The sampler auto-detects the conditioning mask from the model-forward wrapper and skips the rescale at fully-pinned positions (blending it down proportionally at fractional strengths), so this is transparent and needs no separate knob — it only affects the internal guidance math, not anything a preset author configures.
