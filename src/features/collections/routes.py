...
delegating to controller methods. Business logic is in CollectionManager.
"""
...
class CollectionController(BaseController):
    ...
    Handles CRUD for collections plus membership management. Uses
    CollectionManager for business logic. All operations are scoped to the
    current user and to a 'history' | 'library' collection tree.
    """
    ...
    async def list_collections(self, scope: CollectionScope, user: User) -> APIResponse:
        """List all of the user's collections within scope, with generation counts."""
        try:
            collections = self.manager.list_collections(user.id, scope)
    ...
    async def create_collection(self, request: CreateCollectionRequest, user: User) -> APIResponse:
        ...
        try:
            collection = self.manager.create_collection(
                request.name, user.id, request.scope, request.parent_id
            )
            return self.success_response(data={
    ...
    async def rename_collection(
        ...
        """Rename a collection."""
        try:
            collection = self.manager.rename_collection(collection_id, request.name, user.id, request.scope)
            return self.success_response(data={
    ...
    async def move_collection(
        ...
        try:
            collection = self.manager.move_collection(
                collection_id, request.parent_id, user.id, request.scope
            )
            return self.success_response(data={
    ...
    async def bulk_move_collections(
        ...
        try:
            result = self.manager.bulk_move_collections(
                request.collection_ids, request.parent_id, user.id, request.scope
            )
    ...
    async def delete_collection(self, collection_id: str, scope: CollectionScope, user: User) -> APIResponse:
        ...
        try:
            self.manager.delete_collection(collection_id, user.id, scope)
            return self.success_response(data={
    ...
    async def add_members(
        ...
        try:
            added = self.manager.add_members(collection_id, request.generation_ids, user.id, request.scope)
            return self.success_response(data={
    ...
    async def remove_members(
        ...
        try:
            removed = self.manager.remove_members(collection_id, request.generation_ids, user.id, request.scope)
            return self.success_response(data={
    ...
    async def add_upload_members(
        ...
        try:
            added = self.manager.add_upload_members(collection_id, request.upload_ids, user.id, request.scope)
            return self.success_response(data={
    ...
    async def remove_upload_members(
        ...
        try:
            removed = self.manager.remove_upload_members(collection_id, request.upload_ids, user.id, request.scope)
            return self.success_response(data={
    ...
    async def add_prompt_members(
        ...
        try:
            added = self.manager.add_prompt_members(collection_id, request.prompt_ids, user.id, request.scope)
            return self.success_response(data={
    ...
    async def remove_prompt_members(
        ...
        try:
            removed = self.manager.remove_prompt_members(collection_id, request.prompt_ids, user.id, request.scope)
            return self.success_response(data={
